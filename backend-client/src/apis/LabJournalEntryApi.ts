...
export class LabJournalEntryApi extends runtime.BaseAPI {
     ...
     * [v1] Get Lab Journals By Collection
     */
    ...
    /**
     * Applies an RFC 7396 JSON Merge Patch to the lab journal entry. The request body is a partial LabJournalEntry: fields present in the body replace the corresponding fields on the entity, fields absent from the body are left unchanged, and explicit JSON null clears the field. The merged result is then Bean-Validated; constraint violations on the final state return 400. Returns the full updated entity. Accepts both application/merge-patch+json (preferred, per RFC 7396) and application/json in /v1/; future /v2/ APIs will require application/merge-patch+json.
     * [v1] Partially update a lab journal
     */
    async patchLabJournalRaw(requestParameters: PatchLabJournalRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<LabJournalEntry>> {
        if (requestParameters['labJournalEntryId'] == null) {
            throw new runtime.RequiredError(
                'labJournalEntryId',
                'Required parameter "labJournalEntryId" was null or undefined when calling patchLabJournal().'
            );
        }
        ...
        if (requestParameters['labJournalEntry'] == null) {
            throw new runtime.RequiredError(
                'labJournalEntry',
                'Required parameter "labJournalEntry" was null or undefined when calling patchLabJournal().'
            );
        }
     ...
     * [v1] Partially update a lab journal
     */
    async patchLabJournal(requestParameters: PatchLabJournalRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<LabJournalEntry> {
        const response = await this.patchLabJournalRaw(requestParameters, initOverrides);
        return await response.value();
    }

    /**
     * Update a lab journal
     * [v1] Update Lab Journal
     */
