 ...
 *
 * <p>Shape matches {@code de.dlr.shepard.plugin.fileformat.thermography.FileParserPlugin}
 * verbatim (see {@code plugins/fileformat-thermography/...}) — the
 * canonical SPI when it lands will subsume both. This module adds one
 * extension: {@link ParseContext#siblingFiles()}, used by the RoboDK
 * parser to detect a paired {@code .xit} (Spatial Analyzer) file in the
 * same {@code FileContainer} and emit
 * {@code urn:shepard:rdk:companionSpatialAnalyzer}.
 *
 ...
 * service descriptor pointing at {@link RdkTextScrapeParser}.
 *
 * @see RdkTextScrapeParser
 */
public interface FileParserPlugin {
    ...
    interface ParseContext {
         ...
         * being parsed. Used by the RoboDK parser to look up a paired
         * {@code .xit} (Spatial Analyzer) sibling. Default empty —
         * implementations that cannot enumerate siblings simply skip
         * the companion-file annotation, which is allowed by the SPI
         * contract (the predicate is optional).
         */
    ...
    interface AnnotationWriter {
         ...
         * @param predicate     IRI (e.g. {@code urn:shepard:rdk:appVersion})
