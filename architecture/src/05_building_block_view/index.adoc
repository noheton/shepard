...
For decisions and reasoning, check out ADR-008, ADR-010 and ADR-011.

The new module handles persisting timeseries data in a TimescaleDB.
...
The timeseries table is defined by the database migrations in `src/main/java/resources/db/migration` and managed in the code using hibernate entities.
The data point table is also defined by the database schema, but since we want to make full use of TimescaleDB features and performance, we use specific queries for inserting and querying and don't use hibernate entities to interact with it.
