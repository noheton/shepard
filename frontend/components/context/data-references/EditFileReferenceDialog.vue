...
const props = defineProps<{
  /** UUID v7 appId of the FileReference to rename. */
...
async function save() {
  ...
  try {
    ...
    handleError(e as Error, "renaming file reference");
  } finally {
    saving.value = false;
  }
}
        ...
        <v-row class="pt-6">
          <v-col>
            <v-text-field
              ...
              autofocus
              density="comfortable"
              hint="The name shown in the UI and breadcrumbs. Does not rename the underlying file."
              persistent-hint
            />
          </v-col>
        </v-row>
