...
/**
 * REF-EDIT-5 — rename dialog for a StructuredDataReference.
 *
 * Calls PATCH /v2/references/{appId} (V2CONV-A2 unified surface) with
 * { name: newName } on save. Only `name` is mutable on the reference
 * level; the linked container and structured-data OIDs are immutable
 * after creation.
 *
 * Emits:
...
const props = defineProps<{
  /** UUID v7 appId of the StructuredDataReference to rename. */
  structuredDataReferenceAppId: string;
...
async function save() {
  ...
  try {
    ...
    const url = `${v2BaseUrl()}/v2/references/${props.structuredDataReferenceAppId}`;
    ...
    handleError(e as Error, "renaming structured data reference");
  } finally {
    saving.value = false;
  }
}
...
<template>
  <FormDialog
    ...
    title="Rename Structured Data Reference"
    :loading="saving"
