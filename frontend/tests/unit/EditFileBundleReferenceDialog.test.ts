 ...
 * tree. Follows the pattern used in EditFileReferenceDialog.test.ts.
 */
...
describe("EditFileBundleReferenceDialog — validateName", () => {
  ...
  it("is valid when name is non-empty and differs from current", () => {
    const { nameError, isValid } = validateName("new bundle name", "old bundle");
    expect(nameError).toBe("");
...
describe("EditFileBundleReferenceDialog — buildPatchBody", () => {
  ...
  it("returns trimmed name in body", () => {
    expect(buildPatchBody("  hotfire-bundle-revised  ")).toEqual({
      name: "hotfire-bundle-revised",
    });
  });

  it("returns unchanged name when no surrounding whitespace", () => {
    expect(buildPatchBody("MFFD-Q1-AFP-images")).toEqual({
      name: "MFFD-Q1-AFP-images",
    });
  });
});
