/**
 * REF-EDIT-3 — unit tests for EditFileReferenceDialog logic.
 *
 ...
 * pattern used in AnnotationChip.test.ts and ViewRecipeBuilderDialog.test.ts.
 */
...
describe("EditFileReferenceDialog — validateName", () => {
  ...
  it("is invalid when name equals the current name (unchanged)", () => {
    const { nameError, isValid } = validateName("my file", "my file");
...
describe("EditFileReferenceDialog — buildPatchBody", () => {
  ...
  it("returns trimmed name in body", () => {
    expect(buildPatchBody("  calibration run 2  ")).toEqual({
      name: "calibration run 2",
    });
  });

  it("returns unchanged name when no surrounding whitespace", () => {
    expect(buildPatchBody("TR-004-vibration")).toEqual({
      name: "TR-004-vibration",
    });
  });
});

describe("EditFileReferenceDialog — save emits saved on success", () => {
