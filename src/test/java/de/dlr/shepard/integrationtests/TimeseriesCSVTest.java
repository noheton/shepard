...
public class TimeseriesCSVTest extends BaseTestCaseIT {
	...
	public void importTimeseriesCsv()
		...
		given().spec(containerRequestSpec).contentType(ContentType.MULTIPART).multiPart(newFile).when()
				.post(String.format("%s/%d/%s", containerURL, container.getId(), Constants.IMPORT)).then()
				.statusCode(200);
	}
	...
	public void createReference() {
		...
		reference = given().spec(referencesRequestSpec).body(toCreate).when().post().then().statusCode(201).extract()
	...
	public void exportTimeseriesCsv() throws URISyntaxException, IOException {
		...
		var actual = given().spec(referencesRequestSpec).when()
				.get(String.format("%s/%d/%s", referencesURL, reference.getId(), Constants.EXPORT)).then()
