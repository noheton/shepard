...
public class StructuredDataReferenceDAOTest extends BaseTestCase {
	...
	public void findByDataObjectShepardIdTest() {
		...
		String query = "MATCH (d:DataObject)-[hr:has_reference]->(r:StructuredDataReference { deleted: FALSE }) WHERE d.shepardId=11 MATCH path=(r)-[*0..1]-(n) WHERE n.deleted = FALSE OR n.deleted IS NULL RETURN r, nodes(path), relationships(path)";
		when(session.query(StructuredDataReference.class, query, Collections.emptyMap()))
				.thenReturn(List.of(ref, ref2, ref3));
	...
	public void findReachableReferencesByShepardIdStartIdTest() {
		...
		String userName = "user";
		String query = dao.getSearchForReachableReferencesByShepardIdQuery(collectionShepardId, startShepardId,
				userName);
		StructuredDataReference reference = new StructuredDataReference();
		reference.setId(3L);
		when(dao.findByQuery(query, Collections.emptyMap())).thenReturn(List.of(reference));
	...
	public void findReachableReferencesByShepardIdStartIdTraversalRuleTest() {
		...
		String query = dao.getSearchForReachableReferencesByShepardIdQuery(children, collectionShepardId,
