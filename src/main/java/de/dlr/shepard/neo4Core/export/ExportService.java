...
public class ExportService {
	...
	private void fetchAndWriteFileReference(ExportBuilder builder, long referenceId, String username)
			throws IOException {
	...
	private void fetchAndWriteStructuredDataReference(ExportBuilder builder, long referenceId, String username)
		...
		try {
			...
			payloads = structuredDataReferenceService.getAllPayloadsByShepardId(referenceId, username).stream()
					.filter(p -> p.getPayload() != null).toList();
		} catch (InvalidAuthException e) {
	...
	private void writeStructuredDataPayload(ExportBuilder builder, StructuredDataPayload sdp) throws IOException {
		...
		builder.addPayload(sdp.getPayload().getBytes(), filename, sdp.getStructuredData().getName(),
				"application/json");
	}
