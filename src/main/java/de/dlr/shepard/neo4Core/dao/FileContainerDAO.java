...
public class FileContainerDAO extends GenericDAO<FileContainer> {
	...
	public List<FileContainer> findAllFileContainers(QueryParamHelper params) {
		...
		if (params.hasPagination())
			query = String.format("MATCH %s WITH c %s %s", getObjectPart("c", "FileContainer", params.getName()),
					getPaginationPart(params.getPagination()), getReturnPart("c"));
		else
			query = String.format("MATCH %s WITH c %s", getObjectPart("c", "FileContainer", params.getName()),
					getReturnPart("c"));
