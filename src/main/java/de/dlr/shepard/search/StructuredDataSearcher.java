...
public class StructuredDataSearcher implements ISearcher {
	...
	private ResponseBody getStructuredDataResponse(Map<StructuredDataReference, Long> matchingReferences,
	...
	private Map<StructuredDataReference, Long> findMatchingReferences(
			Map<StructuredDataReference, Long> reachableReferences, SearchBody searchBody) {
		...
		for (var reference : reachableReferences.entrySet()) {
			...
			if (mongoQueryResult.first() != null)
				matchingReferences.put(reference.getKey(), reference.getValue());
		}
	...
	private Map<StructuredDataReference, Long> findReachableReferences(SearchBody searchBody, String userName) {
		...
		for (SearchScope searchScope : searchBody.getScopes())
			findReachableReferenceFromScope(searchScope, userName)
