	...
	public ResponseBody search(SearchBody searchBody, String userName) {
		...
		for (SearchScope scope : scopes) {
			...
			// CollectionId given but no DataObjectId
				...
				String selectionQuery = Neo4jEmitter.emitCollectionBasicReferenceSelectionQuery(searchBodyQuery,
			...
			// CollectionId and DataObjectId given
				...
				// no TraversalRules given
					...
					String selectionQuery = Neo4jEmitter.emitCollectionDataObjectReferenceSelectionQuery(scope,
		...
		for (var i = 0; i < resultsSet.size(); i++) {
			resultTriples[i] = new ResultTriple(references[i].getDataObject().getCollection().getId(),
