...
public class Searcher {
	...
	private final BasicReferenceDAO basicReferenceDAO = new BasicReferenceDAO();
	private final StructuredDataReferenceDAO structuredDataReferenceDAO = new StructuredDataReferenceDAO();
	...
	public ResponseBody search(SearchBody searchBody) {
		...
		switch (queryType) {
		case StructuredData:
			ret = getStructuredDataResponse(searchBody);
			break;
		default:
			break;
		}
	...
	private ResponseBody getStructuredDataResponse(SearchBody searchBody) {
	...
	private ResponseBody getStructuredDataResponse(HashSet<StructuredDataReference> reachableReferences,
		...
		for (StructuredDataReference reference : reachableReferences) {
			...
			for (StructuredData structuredData : structuredDatas) {
				...
				mongoStructuredDataIds.add(SearchUtils.makeMongoQueryId(mongoStructuredDataId));
			}
			mongoQuery = "{_id: {$in: " + SearchUtils.makeMongoQueryArray(mongoStructuredDataIds) + "}";
		...
		for (Long matchingReferenceId : matchingReferencesIds) {
			...
			resultTriple.setDataObjectId(basicReferenceDAO.getPredecessorDataObjectId(matchingReferenceId));
	...
	private HashSet<StructuredDataReference> getAllStructuredDataReferencesFromBody(SearchBody searchBody) {
		...
		for (int i = 0; i < searchScopes.length; i++)
			ret.addAll(getAllStructuredaDataReferencesFromScope(searchScopes[i]));
		return ret;
	}
	...
	private HashSet<StructuredDataReference> getAllStructuredaDataReferencesFromScope(SearchScope searchScope) {
		...
		if (traversalRules.length == 0) {
			...
			for (int i = 0; i < traversalRules.length; i++) {
				TraversalRules traversalRule = traversalRules[i];
				List<StructuredDataReference> reachableReferences = structuredDataReferenceDAO
