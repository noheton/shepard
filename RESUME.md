...
**Active arc:** Post-ingest cleanup. The MFFD tapelaying TPS ingest **COMPLETED** 2026-07-21 04:20 UTC (258,670 files, 0 `[err]`, all 3 DNS-casualties re-swept; 258,728 SingletonFileReferences). 2026-07-28 dispatch: 3 agents landed — bridgewelding/semrepo triage (NO data gap; 996614=the tapelaying collection itself, `[bridgewelding]` step is a no-op leftover; 12 empty dupe DOs residue; semrepo 500 = `SparqlConnector.parseJson(null)` upstream bug), GETDO-DETAIL-TARGETED fix (merged `ed76afeaf`, positive edge-type allowlist, 259k→599 db-hits), prune/backfill runbook (plan-only). **Operator chose option 1: hold all mutations** (prune/appid-backfill/agent-edge-backfill deferred). **WIREFIDELITY-IT-CI-GAP** then wired as option-b (PR #2655 merged `4256cdd30`): shipped the real test-compose apoc/n10s plugin fix + a `workflow_dispatch`-only IT skeleton (non-gating); greening the dormant `@QuarkusIntegrationTest` suite needs full prod-config provisioning (valid OIDC JWT key) → tracked as **INTEGRATION-IT-PROVISION**. GETDO-DETAIL-TARGETED byte-compat stays on the static allowlist proof. **Then operator picked 1-4 and all four shipped the same day:** (1) **backend redeployed** (GETDO + SEMREPO-500 fix + the July APISIMP/TIFF/MP4 backlog) — smoke **26/26** after fixing a stale check (`runtime-toggles` → `/v2/admin/config/feature-toggles`, an APISIMP rename, not a regression); GETDO **empirically verified live: 258,789 → 43 db-hits** on the 258,751-ref DO, detail page now **200 in ~3.3 s** (was unopenable). (2) **k6 LARGE-DATA baseline** run → `aidocs/agent-findings/k6-large-data-baseline-2026-07-28.md`; **found a ~1.8 s constant cost on EVERY DO detail read** (un-indexed `shepardId`) → `GETDO-DETAIL-SHEPARDID-INDEX` raised MINOR→**MAJOR**, now the dominant HTTP-layer cost. (3) **12 empty dupe DOs soft-deleted** (one transient 403, retried) + **MFFD-POST-RETRY-NONIDEMPOTENT fixed** (ReadTimeout no longer re-sends a POST; backend has no Idempotency-Key so the agent's suggested fix would have been a no-op). (4) **Both additive backfills executed**: 567,658 `:ShepardFile` appIds (<45 s) + 3,008,314 agent edges (<60 s), both verified 0-remaining — **but the runbook's agent-edge query was NOT safe as written** (PROFILE: drove from the `:User` supernode, 2.18M fixed db-hits/200 rows) and was corrected to a split pattern (~4.5/row, ~2,200× less) before running. **PRUNE still NOT RUN** — remains gated on an offline `neo4j-admin` dump + boundary snapshots on the 3 tombstone-holding collections. Next: `GETDO-DETAIL-SHEPARDID-INDEX`, `INTEGRATION-IT-PROVISION`, `PYTHON-API-IT-RED`, the prune when you want it.
...
**Second wave (2026-07-20, same day) — supernode sweep + 6 more fixes, ~74% ingest:** the `DEPTH-ENTITY-SUPERNODE-SWEEP` agent found 6 net-new load sites; fixed via 3 parallel worktree agents + me: **SUPERNODE-F1-VERSION** (CRITICAL, `3c67a2a44` — EVERYTHING→OUTGOING, removed nested O(versions×25k) re-drag; **the SNAPSHOT-ASYNC-CAPTURE 502 root cause**; live-proven 16,748→2 rows), **F2-COLLECTION-DETAIL** (`868fbaf67`, 8,483-DO detail → 300 B/~0.5s), **F3-CONTAINER-LINKED-DO** (`af965b546`), **F5/F6** (`cbd75090e` — tryBackfillLocalUser findLight + findLightByAppId on DO-list filters), **GETCURRENTUSER-GLOBAL-DEPTH0** (`c4c0ad338` — getCurrentUser depth-0 + getCurrentUserWithCollections for the 3 UserIO wire sites). All integrated (283-test run), deployed, live-verified; byte-compat held (/me + api-keys + subscriptions). **KEY LESSON ([[project-ogm-load-depth-supernode]]):** all 3 agents independently REJECTED the depth-0 flip (NPEs IOs / nulls the v1 wire) → exclude only the fan-out edge instead. And empirically the `:User` depth-1 `session.load` is ~10ms even on the 3.19M-edge user (OGM fetches only MAPPED rels) — so the getCurrentUser/User-supernode fixes are safe but LOW-impact; the real wins were the DataObject/Version/Collection `findByQuery`+`getReturnPart` loads. **Deferred (k6-gated):** broad findByAppId/list loaders, GETDO-DETAIL-TARGETED, SUPERNODE-F2-WATCHSERVICE. **k6:** `LARGE-DATA-K6-COVERAGE` scenarios authored (`scripts/perf/k6-endpoints.js`, env-gated LARGE_*), **run after ingest completes**.

**Ingest ops state:** running in tmux `mffd-tapelaying-20260710b`, **WORKERS=8**, ~74% done (~193k/258k refs), ETA a few h. Supervisor `.relaunch-tapelaying-w8.sh` (resume-only, no `--reset`); stop via `touch /tmp/mffd-runner.stop` + SIGINT. **Restart gotcha (bit me twice):** SIGKILL leaves a stale `.mffd-import.lock` (blocks next start, runner backoff-loops) AND each restart pays a resume-skip re-scan of the 129k done files (~2-4 min before uploads resume) — prefer SIGINT (graceful, releases lock) and avoid needless restarts. Scratch `CASCADE-GATE-fixA` collection deleted; 0 probe refs left on the real DO.
