...
**Completeness note (DNS blips):** the ingest host had a brief `Temporary failure in name resolution` for `shepard.nuclide.systems` ~4h44m into the w8 run — hit all workers at once, exhausted one file's 5-attempt/60s backoff → 1 `[err]` (`Track_117/Run_23349/TPS raw data.8`) out of 23k. Failed files are left **unmarked** in the state file, so **a resume pass after the run completes re-attempts every `[err]` casualty** — that final sweep is required for completeness (`feedback_completeness_nonnegotiable`). Not a worker-count issue (DNS is orthogonal to concurrency); do NOT dial down for it.
...
**Frontend redeployed 2026-07-18 21:13** (new image, smoke **26/26 PASS**) — ships both new UIRULE-NO-MANUAL-IDS pickers (WatchedContainersPanel + HdfReferencesPane container-appId → searchable picker). Gate 5 (deploy+smoke) ✅. **Gate 6 (interactive Playwright @4K) — mostly unblocked, one step open.** Shipped the **Bearer-token e2e helper** the block needed (`e2e/tests/helpers/api.ts` + `api-bearer-helper.spec.ts`, `9f07803c9`): `sessionToken(page)` extracts the live nuxt-auth session token (via `/api/auth/session`) — resolves to the same backend user the browser acts as — so `createFixtureCollection()` mints a session-OWNED collection (create 201 → self-cleaning delete 204 after the ~2 s PERM-SEED-V1 lag). This solved: cookies→401 (now Bearer→200), row-click-nav (skip it, `goto` the fixture appId), and the alice-editability unknown (create her own → `isAllowedToEdit` true, Edit/ADD affordances present). The 4K watch-picker spec then successfully: logs in → creates fixture → navigates → expands the collapsed "Watched containers" `ExpansionPanelItem` → finds + clicks the editor-only "Add watch" (present + enabled). **The ONE remaining block:** clicking "Add watch" does not surface the add-form under Playwright — `.add-form` + the `watch-container-autocomplete` testid never render in-test (normal + force click), despite the button being present/enabled and the testid confirmed IN the deployed bundle. Cause not isolated (page dual-renders basic/advanced; possible `@click` handler not firing in the test tree) — needs trace-level debugging. NOT a picker defect (unit-tested, typecheck/lint green, deployed, smoke 26/26). The red spec was NOT committed; the Bearer helper (the reusable win) was. Tracked in UIRULE-NO-MANUAL-IDS `Follow-up: Playwright 4K`.
