...
| `fileformat-cad` | `plugins/fileformat-cad/` (`shepard-plugin-fileformat-cad`) | Phase-1 metadata parser for CAD exchange formats used in aerospace composite manufacturing. Accepts `.step`/`.stp` (STEP ISO 10303-21 — HEADER + partial AP242 DATA), `.3dxml` (Dassault 3DXML — Manifest + product structure), `.jt` (JT ISO 14306 — format detection only), `.obj` (Wavefront mesh — vertex/face count). Emits `urn:shepard:cad:*` and `urn:shepard:mffd:cad:*` `:SemanticAnnotation`s on upload. No REST endpoints, no admin config. Phase 2 (CAD-RENDER-1) adds glTF preview via `ViewRecipeRenderer`. See `plugins/fileformat-cad/docs/`. | CAD-STEP-1 |
