...
| `jupyter` | `plugins/jupyter/` (`shepard-plugin-jupyter`) | JupyterHub link-out integration — "Open in JupyterHub" action on `.ipynb` FileReference rows in the unified data-references table. Path-mounted at `/jupyterhub` on the existing Shepard host (no new DNS / TLS). Ships a JupyterHub 5.x sidecar (`compose-profile.yml`) with a `?file=<url>` pre-spawn hook that fetches the notebook bytes into the user's workspace before the kernel starts. Admin-configurable at runtime: `:JupyterConfig` singleton + `GET/PATCH /v2/admin/jupyter/config` + `shepard-admin jupyter {status,enable,disable,set-hub-url}` CLI parity. **Disabled by default** (`shepard.jupyter.enabled=false`). Reference: [plugins/jupyter/docs/reference.md](../../plugins/jupyter/docs/reference.md). Quickstart: [plugins/jupyter/docs/quickstart.md](../../plugins/jupyter/docs/quickstart.md). Install guide: [plugins/jupyter/docs/install.md](../../plugins/jupyter/docs/install.md). | J1e / J2a |
