...
extensions — see [`aidocs/56`](https://github.com/noheton/shepard/blob/main/aidocs/platform/56-v2-api-simplification-output-profiles-mcp.md)).
...
`Collection` and `DataObject` are both reachable on the new shelf:

| Entity | Upstream-frozen `/v1` shape | Fork `/v2` shape |
|---|---|---|
| **Collection** list | `GET /shepard/api/collections?page=0&size=50` | `GET /v2/collections?page=0&size=50` |
| Collection get one | `GET /shepard/api/collections/{long-id}` | `GET /v2/collections/{appId}` |
| Collection create | `POST /shepard/api/collections` | `POST /v2/collections` |
| Collection update (partial) | `PATCH /shepard/api/collections/{long-id}` | `PATCH /v2/collections/{appId}` (RFC 7396 merge-patch) |
| Collection delete | `DELETE /shepard/api/collections/{long-id}` | `DELETE /v2/collections/{appId}` |
| **DataObject** list | `GET /shepard/api/collections/{long}/dataObjects?page=0&size=50` | `GET /v2/collections/{collectionAppId}/data-objects?page=0&size=50` |
| DataObject get one | `GET /shepard/api/collections/{long}/dataObjects/{long}` | `GET /v2/collections/{collectionAppId}/data-objects/{dataObjectAppId}` |
| DataObject create | `POST /shepard/api/collections/{long}/dataObjects` | `POST /v2/collections/{collectionAppId}/data-objects` |
| DataObject update (partial) | `PATCH /shepard/api/collections/{long}/dataObjects/{long}` | `PATCH /v2/collections/{collectionAppId}/data-objects/{dataObjectAppId}` |
| DataObject delete | `DELETE /shepard/api/collections/{long}/dataObjects/{long}` | `DELETE /v2/collections/{collectionAppId}/data-objects/{dataObjectAppId}` |

Every `/v1` response already carries the `appId` field
...
The two shelves share the same services underneath — each `/v2`
resource translates `appId → ogmId` at the boundary via the
request-scoped `EntityIdResolver`.

The three container kinds (`FileContainer`, `TimeseriesContainer`,
`StructuredDataContainer`) follow the same shape in the next slice
of L2d Phase A (see `aidocs/44`).
...
This is the **new baseline** ([ADR-0022](https://github.com/noheton/shepard/blob/main/aidocs/platform/63-architecture-decision-log.md#adr-0022)),
...
This is the **legacy still-maintained** path ([ADR-0022](https://github.com/noheton/shepard/blob/main/aidocs/platform/63-architecture-decision-log.md#adr-0022),
...
generated clients — see [`aidocs/27`](https://github.com/noheton/shepard/blob/main/aidocs/ops/27-convenience-clients-design.md)
...
- [`aidocs/57 §4`](https://github.com/noheton/shepard/blob/main/aidocs/ops/57-openapi-client-generator-evaluation.md#4-recommendation) — full generator evaluation.
- [`aidocs/63` ADR-0022](https://github.com/noheton/shepard/blob/main/aidocs/platform/63-architecture-decision-log.md#adr-0022) — dual-generator decision record.
- [`aidocs/27`](https://github.com/noheton/shepard/blob/main/aidocs/ops/27-convenience-clients-design.md) — convenience-wrapper design.
