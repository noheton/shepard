...
audience: admin
permalink: /admin/system-requirements/
---

# System requirements

Starting-point requirements for a small to medium production deployment.
Real-world sizing depends on data volumes (timeseries cardinality, file sizes,
spatial extent) and concurrent users — treat the numbers below as a floor, not
a ceiling.
...
| Garage S3 (optional, via plugin) | `dxflrs/garage:v1.0.1` |
| HSDS (optional, HDF5 sidecar) | `hdfgroup/hsds:v0.9.5` |
...
IdP. See [Authentication]({{ '/admin/auth/' | relative_url }}) for the full
role-mapping model.
