...
> **⚠ Persistence is HOST BIND MOUNTS, not named volumes.** This deployment
> binds the data dirs to host paths (`docker-compose.yml`): Neo4j →
> `/opt/shepard/neo4j/data`, MongoDB → `/opt/shepard/mongodb/db`, TimescaleDB →
> `/opt/shepard/timescaledb`. Only **Garage** uses named volumes
> (`infrastructure_garage_data` / `_meta`). The old `docker volume rm
> infrastructure_*-data` commands below are WRONG for this host — wipe the bind
> dirs instead. Stop the data-plane containers FIRST (Phase 1), or you unlink
> files while they're open and the container re-creates a fresh cluster
> underneath you.
...
```bash
# With neo4j stopped (Phase 1), wipe the host bind-mount contents.
rm -rf /opt/shepard/neo4j/data/databases /opt/shepard/neo4j/data/transactions
...
```bash
# Single TimescaleDB cluster (no separate "postgres" container). Host bind dir.
rm -rf /opt/shepard/timescaledb/*
# Flyway migrations rerun on startup (V1.* + V2.*); fresh schema.
# The initdb script (docker-entrypoint-initdb.d/postgres/00-init-postgres-db.sh)
# re-runs on the empty dir and recreates the shepard user, pgbouncer auth fn,
# and the pgcrypto / pg_stat_statements extensions (see §2.6 gotcha 1).
```

### 2.3 MongoDB (GridFS file payloads, journal entries, SD payloads)

```bash
rm -rf /opt/shepard/mongodb/db/*
```

### 2.4 Garage S3 (file payloads, OME-Zarr volumes) — named volumes

```bash
# Garage is the ONLY substrate on named volumes. Stop + remove the exited
# container first (it holds the volume), then rm the volumes.
docker compose stop garage && docker rm shepard-garage 2>/dev/null || true
docker volume rm infrastructure_garage_data infrastructure_garage_meta
# NOTE: default storage is POSIX (shepard.plugins.file-s3.enabled=false), so on a
# fresh instance you do NOT need to recreate the Garage bucket unless an operator
# has flipped file-s3 on at runtime. Skip §3.1 if storage is POSIX.
```

### 2.5 Verify

```bash
du -sh /opt/shepard/neo4j/data /opt/shepard/mongodb/db /opt/shepard/timescaledb
# Expected: near-empty (a few hundred bytes / KB) — dirs are repopulated on `up`.
docker volume ls | grep -E "infrastructure_garage_(data|meta)"   # expected: empty
```

### 2.6 Fresh-DB boot gotchas (fixed 2026-06-17 — now pre-empted)

A from-scratch cluster exposed three startup blockers that an *incremental*
upgrade never hits (the migrations had only ever run against pre-existing data).
All three are now fixed in-repo so a clean reset boots one-shot; documented here
for awareness if you reset an older checkout:

1. **`pgcrypto` / `pg_stat_statements` extension privilege.** Flyway connects as
   the unprivileged `shepard` user and `V1.11.0` does `CREATE EXTENSION pgcrypto`
   → *permission denied*. **Fix:** `00-init-postgres-db.sh` now pre-creates both
   extensions as the bootstrap superuser. Manual recovery if needed:
   `docker exec infrastructure-timescaledb-1 psql -U postgres -d postgres -c
   "CREATE EXTENSION IF NOT EXISTS pgcrypto; CREATE EXTENSION IF NOT EXISTS pg_stat_statements;"`
2. **`refresh_continuous_aggregate()` inside a transaction.** `V1.17.0` calls it
   to backfill the hourly CAgg, but TimescaleDB forbids it in a txn block and
   Flyway wraps every script. **Fix:** sidecar
   `V1.17.0__*.sql.conf` with `executeInTransaction=false`.
3. **Stale orphan-permissions `default-owner`.** `docker-compose.override.yml`
   pinned `SHEPARD_PERMISSIONS_DEFAULT_OWNER` to a user UUID that the wipe
   deleted; the backfill guard aborts startup with *"no User with that username
   exists in Neo4j"*. **Fix:** the override is now unset (empty = skip backfill,
   correct for an empty graph). Re-set to a real instance-admin sub only if a
   future backfill is actually needed.
