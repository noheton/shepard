...
import { addManySpatialDataPoints } from "./spatial-helper";
import { fail } from "k6";
...
export function uploadData() {
  ...
  if (NUMBER_OF_DATAPOINTS <= UPLOAD_BATCH_SIZE) {
    addManySpatialDataPoints(CONTAINER_ID, NUMBER_OF_DATAPOINTS, NUMBER_OF_MEASUREMENTS);
  } else {
    for (let i = 0; i < Math.floor(NUMBER_OF_DATAPOINTS / UPLOAD_BATCH_SIZE); i++) {
      ...
      const result = addManySpatialDataPoints(CONTAINER_ID, UPLOAD_BATCH_SIZE, NUMBER_OF_MEASUREMENTS);
