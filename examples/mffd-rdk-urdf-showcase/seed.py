...
RDK-PARSE-1:
  ...
  2. A KUKA KR210 L150 URDF + its 7 Collada meshes are uploaded as
     FileReferences for provenance. (The runtime fetch path is
     ``/urdf-samples/kr210/...`` from the frontend public dir — see
     ``urdf/SOURCES.md``.)
...
from the open-source ros-industrial/kuka_experimental package
(Apache-2.0). See ``SHOWCASE.md`` for the honest caveats: tier-1
scrape is metadata-only (no kinematics); the URDF is a model
substitution (KR210 L150 stands in for the actual MFFD KR270 R2700);
the trajectory is synthetic but joint-limit-respecting and plausible.
"""
...
COLLECTION_DESCRIPTION = (
    ...
    "`urn:shepard:rdk:*` annotations automatically); (b) a KUKA KR210 L150 "
    "URDF + meshes sourced from the open-source ros-industrial/"
    "kuka_experimental package (Apache-2.0; substitutes for the real MFFD "
    "KR270 R2700 — see urdf/SOURCES.md § Caveat); (c) a synthetic 30-second "
    "6-joint AFP-ply-layup trajectory with `urn:shepard:urdf:joint` "
    "annotations the UrdfChannelPicker auto-binds to. **Phase 2** "
    "(`RdkToUrdfExporter` sidecar — RoboDK SDK licence required) would "
    "remove the URDF substitution; queued in the URDF-WEBVIEW-1 backlog."
)

COLLECTION_ATTRIBUTES = {
    "topic": "RDK→URDF→animator end-to-end",
    "robot": "KUKA KR210 L150 (substitute for KR270 R2700)",
    "url:urdf-samples": "/urdf-samples/kr210/kuka_kr210_support/urdf/kr210l150.urdf",
    "url:source-rdk": "examples/mffd-showcase/raw-data/mffd-data/cell/MFZ.rdk",
    "license:urdf": "Apache-2.0 (ros-industrial/kuka_experimental)",
...
URDF_PACKAGE_DIR = "/urdf-samples/kr210"

# The 6 movable KR210 joints, in URDF declaration order.
...
# Template
TEMPLATE_NAME = "MFFD RDK→URDF AFP Layup (KR210 + joint-trajectory)"
...
FE_MESH_DIR     = SEED_DIR.parent.parent / "frontend" / "public" / "urdf-samples" / "kr210" / "kuka_kr210_support" / "meshes" / "kr210l150" / "visual"
...
def ensure_view_recipe_template(host: str, api_key: str, joint_channel_ids: dict[str, str]) -> str | None:
    ...
    body = {
        ...
        "label":            "MFFD AFP Ply 5 layup (KR210 stand-in)",
    }
    payload = {
        "name":         TEMPLATE_NAME,
        "templateKind": TEMPLATE_KIND,
        "body":         json.dumps(body),
        "description":  ("Drives UrdfView with the KR210 KUKA URDF + the AFP "
                         "joint trajectory's 6 joint-angle channels. Channel "
                         "binding via the urn:shepard:urdf:joint annotation."),
        "tags":         ["mffd", "urdf", "kr210", "view-recipe", "afp"],
    }
    v2 = _v2_base(host)
...
def seed(apis: Apis) -> None:
    ...
    do_urdf = ensure_data_object(
        apis, coll, DO_URDF,
        "KUKA KR210 L150 URDF (ros-industrial/kuka_experimental, "
        "Apache-2.0). Substitute for the real MFFD KR270 R2700 cell robot "
        "(no public URDF available; see urdf/SOURCES.md § Caveat). The 7 "
        "Collada visual meshes are referenced via package:// paths and "
        "served from the frontend public dir at /urdf-samples/kr210/.",
        {
            "robot-family": "KUKA KR210 L150",
            "represents":   "KUKA KR270 R2700 (MFFD cell)",
            "license":      "Apache-2.0 (ros-industrial)",
            "url:public":   URDF_PUBLIC_URL,
            "packagePath":  URDF_PACKAGE_DIR,
            "joint-count":  "6 revolute (joint_a1 .. joint_a6)",
        },
    )
