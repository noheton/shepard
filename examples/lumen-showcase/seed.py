...
# ---- REF-EDIT-1: channel-selection prefill annotations --------------------

# Key sensor channels logged on every hot-fire test run.
# Matches the synthetic channel names generated by data/generate.py.
_HOTFIRE_CHANNEL_SELECTION_HINT = json.dumps({
    "channels": [
        {"measurement": "thrust",       "device": "loadcell",    "location": "ENGINE",   "symbolicName": "TH1",  "field": "value"},
        {"measurement": "pressure",     "device": "transducer",  "location": "CHAMBER",  "symbolicName": "PC1",  "field": "bar"},
        {"measurement": "temperature",  "device": "thermocouple","location": "TURBINE",  "symbolicName": "TT1",  "field": "celsius"},
        {"measurement": "vibration",    "device": "accelerometer","location": "TURBOPUMP","symbolicName": "VIB1", "field": "rms"},
    ],
    "windowDurationNs": 30_000_000_000,  # 30 s — covers the standard hot-fire burn window
})


def best_effort_channel_selection_annotations(
    apis: "Apis",
    coll: "Collection",
    runs: "dict[int, DataObject]",
) -> None:
    """Seed a `urn:shepard:reference:channelSelection` annotation on each test-run
    DataObject.

    The Create TimeseriesReference dialog reads this annotation from the parent
    DataObject and prefills the channel picker with the four primary sensors and
    a 30 s default window. Idempotent: duplicate-annotation errors are swallowed.

    Requires the INTERNAL semantic repository (V49 migration).
    """
    PRED = "urn:shepard:reference:channelSelection"
    # SemanticAnnotation uses `valueName` for free-text values (no valueIRI).
    from dlr_shepard_backend_client import SemanticAnnotation as _SA
    for run_idx, do in runs.items():
        ann = _SA(
            propertyIRI=PRED,
            valueName=_HOTFIRE_CHANNEL_SELECTION_HINT,
        )
        try:
            apis.annotation.create_data_object_annotation(
                collection_id=coll.id,
            ...
            _log("OK", do.name, "channelSelection annotation seeded")
        except Exception as exc:
            err_str = str(exc)[:80]
            if "already exists" in err_str.lower() or "409" in err_str or "duplicate" in err_str.lower():
                _log("SKIP", do.name, "channelSelection annotation (already exists)")
            else:
                _log("SKIP", do.name, f"channelSelection annotation: {err_str}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    best_effort_channel_selection_annotations(apis, coll, runs)
