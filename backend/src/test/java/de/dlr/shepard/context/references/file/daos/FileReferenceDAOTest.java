...
public class FileReferenceDAOTest extends BaseTestCase {
  ...
  public void findByDataObjectTest() {
    ...
    // L2c: WHERE ID(d) flipped to WHERE d.appId; the resolver translates
    // the internal long to the entity's appId at the DAO boundary.
    when(entityIdResolver.resolveAppId(1L)).thenReturn("appid-do-1");
    var query =
      """
      MATCH (d:DataObject)-[hr:has_reference]->(r:FileReference { deleted: FALSE }) WHERE d.appId=$dataObjectAppId \
