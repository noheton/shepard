...
class PayloadVersionRestTest {
  ...
  void listVersions_returns200WithEmptyArrayWhenNoVersionsExist() {
    ...
    when(dao.findByContainerAndName(CONTAINER_APP_ID, ORIGINAL_NAME)).thenReturn(List.of());

    Response resp = resource.listVersions(CONTAINER_APP_ID, ORIGINAL_NAME);
  ...
  void listVersions_returns200WithOneVersionAfterSingleUpload() {
    when(service.getContainerByAppId(CONTAINER_APP_ID)).thenReturn(container());
    PayloadVersion v1 = buildVersion("appId-1", 1L, "oid1",
      "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855", 4096L, "alice");
    when(dao.findByContainerAndName(CONTAINER_APP_ID, ORIGINAL_NAME)).thenReturn(List.of(v1));
  ...
  void listVersions_returns200WithTwoVersionsAfterReUpload() {
    ...
    when(dao.findByContainerAndName(CONTAINER_APP_ID, ORIGINAL_NAME)).thenReturn(List.of(v1, v2));
  ...
  void listVersions_callsPermissionCheckViaGetContainerByAppId() {
    ...
    resource.listVersions(CONTAINER_APP_ID, ORIGINAL_NAME);
