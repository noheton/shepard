...
class QueryHelper {
  ...
  public <T> T create(Node node, Class<T> type) {
    var statement = Cypher.create(node).returning(node).build();
    return queryResults(statement, type).getFirst();
  }
  ...
  public <T> T loadSingle(Long id, Class<T> type) {
