...
@RequestScoped
public class InfluxCsvConverter {

  public InputStream convertToCsv(List<InfluxTimeseriesPayload> payloads) throws IOException {
    ...
    var writer = new StatefulBeanToCsvBuilder<InfluxTimeseriesCsv>(streamWriter).withApplyQuotesToAll(false).build();
  ...
  public List<InfluxTimeseriesPayload> convertToPayload(InputStream stream) throws IOException {
    var reader = new InputStreamReader(stream);
    var cb = new CsvToBeanBuilder<InfluxTimeseriesCsv>(reader)
      .withType(InfluxTimeseriesCsv.class)
    ...
    List<InfluxTimeseriesCsv> result = cb.parse();
    reader.close();
    return convertCsvToPayload(result);
  }

  private List<InfluxTimeseriesCsv> convertPayloadToCsv(InfluxTimeseriesPayload payload) {
    var ts = payload.getTimeseries();
    var result = new ArrayList<InfluxTimeseriesCsv>(payload.getPoints().size());
    for (var p : payload.getPoints()) {
      var tsc = new InfluxTimeseriesCsv(
  ...
  private List<InfluxTimeseriesPayload> convertCsvToPayload(List<InfluxTimeseriesCsv> inputList) {
    var result = new HashMap<Integer, InfluxTimeseriesPayload>();
