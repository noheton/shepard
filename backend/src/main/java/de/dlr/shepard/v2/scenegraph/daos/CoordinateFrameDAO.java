 ...
 * pattern). Also overrides {@link #findByParentAppId} to avoid the
...
public class CoordinateFrameDAO extends GenericDAO<CoordinateFrame> {
  ...
  /**
   * Load all {@link CoordinateFrame} nodes using a live session.
   *
   * <p>Returns an empty collection when the session factory is not
   * yet available (fail-soft on reads).
   */
  @Override
  public Collection<CoordinateFrame> findAll() {
    Session live = NeoConnector.getInstance().getNeo4jSession();
    if (live == null) {
      return Collections.emptyList();
    }
    return live.loadAll(CoordinateFrame.class, DEPTH_ENTITY);
  }

  /**
   * Find all frames whose {@code parentFrameAppId} equals the given
   * value. Returns an empty collection when no matches exist or when
   * the session factory is not yet available.
   *
