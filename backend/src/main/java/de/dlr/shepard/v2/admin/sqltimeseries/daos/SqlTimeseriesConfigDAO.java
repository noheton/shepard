...
public class SqlTimeseriesConfigDAO extends GenericDAO<SqlTimeseriesConfig> {
  ...
  public SqlTimeseriesConfig findSingleton() {
    ...
    Collection<SqlTimeseriesConfig> all = live.loadAll(SqlTimeseriesConfig.class, 1);
   ...
   */
  @Override
  public SqlTimeseriesConfig createOrUpdate(SqlTimeseriesConfig entity) {
    Session live = NeoConnector.getInstance().getNeo4jSession();
    if (live == null) {
      throw new IllegalStateException("Neo4j session unavailable — cannot persist :SqlTimeseriesConfig");
    }
