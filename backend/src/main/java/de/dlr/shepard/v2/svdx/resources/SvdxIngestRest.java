 ...
 * tier-1 parser ({@code MFFD-PLUGIN-SVDX-1}). The CSV ingest path
 * handled here remains the cleanest route when a paired
 * TwinCAT-Scope-Export-Tool {@code .csv} sibling exists — for the
 * standalone {@code .svdx} case the binary sample region is now
 * decoded by {@code SvdxBinaryParser} (see
 * {@code MFFD-PLUGIN-SVDX-BINARY-PARSER-1}); a follow-up will add a
 * {@code POST /v2/svdx/ingest-binary} that walks that decoder's
 * output through the same {@code TimeseriesService.saveDataPoints}
 * sink, reusing the idempotency contract below.
 */
@Path("/v2/svdx")
@RequestScoped
...
public class SvdxIngestRest {
  ...
  public Response ingest(
    ...
    try {
      SvdxIngestResponseIO out = svc.ingest(req, caller, overwrite);
