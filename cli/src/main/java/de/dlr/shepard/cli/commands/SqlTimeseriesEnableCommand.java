...
/**
 * FTOGGLE-CLI-PARITY-1 — {@code shepard-admin sql-timeseries enable}.
 * Flips the runtime SQL timeseries toggle via
 * {@code PATCH /v2/admin/sql-timeseries/config} with {@code {"enabled": true}}.
 *
...
@Command(
  ...
  description = "Enable the SQL timeseries query surface at runtime (sets enabled=true)."
)
public final class SqlTimeseriesEnableCommand extends AbstractCommand {
  ...
  protected Integer run() {
    ...
    out().println("SQL-TIMESERIES — enabled");
    out().println();
