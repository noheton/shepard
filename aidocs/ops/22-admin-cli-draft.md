...
- **MongoDB transactions** — see the `mongoDB` module (3 files,
  `aidocs/archive/06-code-quality.md` line 71). No explicit lock taken in
...
- **TimescaleDB advisory locks** — Flyway acquires its `flyway_schema_history`
  ...
  migration is called out in `aidocs/data/12-timescaledb-performance-analysis.md`
  row 16) the lock can persist. `MigrationsRunner.java` does not currently
  release on shutdown.
- **`FileContainerService`** race (issue #721, `aidocs/archive/03-issues-status.md`
...
path. Security finding H2 (`aidocs/archive/07-security-issues.md`) calls out
...
`aidocs/data/35-hdf5-hsds-implementation-design.md`). For these, the CLI
...
| `hdf` | `hdf` | `hsds` | `aidocs/data/35-hdf5-hsds-implementation-design.md` |
...
| Hint | New admin endpoint; metrics already exposed at `/shepard/doc/metrics/prometheus` per `aidocs/archive/01-repo-overview.md:78` — `stats cache-hit-rate` could just scrape Prometheus |
| Idempotent | Yes |
| Size | S |
...
4. **Storage backends** — the conditional section. If `spatial`
   ...
   `aidocs/data/35-hdf5-hsds-implementation-design.md` §3.
...
| `export collection` | `GET /collections` (existing, with paging) | Cursor-paged picker; once selected, the export-options form auto-fills with sane defaults from `aidocs/workflows/31-rocrate-export-optimisation.md` |
...
2. The team's CI is already Java/Maven-fluent
   (`aidocs/archive/01-repo-overview.md:55-57`); adding a fourth Maven module
   is incremental.
...
- **Homebrew tap** — explicitly **not** in scope. shepard targets DLR
  ...
  (per the platform constraint at `aidocs/archive/01-repo-overview.md:16`,
  "On-premises only").
...
- **Integration test against `infrastructure-local/` compose stack.**
  ...
  (`@QuarkusIntegrationTest` per `aidocs/archive/01-repo-overview.md:50`).
...
- Permission TTL gap (motivates 4.3): `aidocs/archive/07-security-issues.md`
  finding H2; `UserLastSeenCache.java:8` (30 min).
- File-container race (motivates 4.2): issue #721 per
  `aidocs/archive/03-issues-status.md` line 45.
- Performance backlog that interacts with 4.4 / 4.10:
  `aidocs/data/12-timescaledb-performance-analysis.md` rows 15 and 16 (no
