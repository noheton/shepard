...
- `aidocs/16-dispatcher-backlog.md` — R2 (`be0eb26`), R2b (`60a3ea1`),
  ...
  P13 / P22 / P23; A4 / A4d.
- `aidocs/platform/19-architecture-feedback.md` §2.6 (heap-buffered hot reads),
  §2.8 (no correlation id — the export will benefit from request-id
  threading once landed).
- `aidocs/platform/26-crud-consistency.md` — adding `/export-jobs/{id}` is one
  more resource type; align verbs (POST create, GET status, DELETE
  cancel) with the table.
- `aidocs/ops/28-paradigms-and-clients-synthesis.md` §3.1 (Surface A REST
