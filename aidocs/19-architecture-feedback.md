...
re-statement of `aidocs/01-repo-overview.md`. Strengths first, then
fragilities prioritised by blast radius, then the seams that aren't
seams yet, then risks specific to the proposals in
`aidocs/13-search-improvements.md` and `aidocs/14-semantic-improvements.md`,
...
- Companion docs:
  `aidocs/12-timescaledb-performance-analysis.md` (timeseries / read
  path; §11 for identifier discipline),
  `aidocs/13-search-improvements.md` (unified search),
  `aidocs/14-semantic-improvements.md` (annotation generalisation,
  triplestore).
- Backlog: `aidocs/16-dispatcher-backlog.md` (A0, A1*, A3*, A4*, P2,
  P3*, P4, S1; F1/F2/F3 proposed in §5 above).
- Issue context: `aidocs/02-cluster-map.md` (clusters E, F, H);
  `aidocs/07-security-issues.md` (C1–C5, H1–H8);
  `aidocs/11-implementation-plan.md` (Phase 1A/1B).
