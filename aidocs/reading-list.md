...
| 2026-05-23 | 2017-12-05 DLR@UBC HRI Demo deck (DLR-internal primary source) | Workshop deck same testbed (KUKA + Hololens + Pozyx + V-REP + OPC-UA) | Doc 102 | Chronological anchor: same 2017-12-05 day as Vision_Datenmanagement PPTX — HRI + data-mgmt thread emerge together | cited |
...
### Cross-instance provenance UI design (2026-05-23)

| Date | Source | Topic | Surface | Why interesting | Status |
|---|---|---|---|---|---|
| 2026-05-23 | [W3C PROV-O](https://www.w3.org/TR/prov-o/) | Canonical PROV-O vocabulary — `prov:wasAttributedTo`, `prov:wasGeneratedBy`, `prov:wasDerivedFrom`, `prov:Agent` | `aidocs/frontend/100 §6` | Target substrate for the FE-PROV-AGGREGATOR's M4I-b migration — opaque `source_*` attrs migrate to typed PROV-O edges, aggregator response stays unchanged | skimmed |
| 2026-05-23 | [W3C DCAT 3](https://www.w3.org/TR/vocab-dcat-3/) | Data Catalog Vocabulary — `dct:creator`, `dct:publisher`, `dct:provenance`, `prov:wasGeneratedBy` patterns for cross-catalog datasets | `aidocs/frontend/100` cross-instance attribution | DCAT 3's "dataset that lives in multiple catalogs" model maps onto the source-instance / dest-instance split; potential alignment target for the aggregator's link-out (`dcat:landingPage`) | unread |
| 2026-05-23 | [Metadata4Ing 1.4.0](https://w3id.org/nfdi4ing/metadata4ing/1.4.0/) | M4I canonical predicates — `m4i:realizesMethod`, `obo:RO_0002233`, `obo:RO_0002234` | `aidocs/frontend/100 §6` migration path | The aggregator endpoint becomes the M4I-b migration seam — when the importer rewrites `source_*` attrs as typed m4i triples, the wire response is unchanged | skimmed |
| 2026-05-23 | [DataCite Metadata Schema 4.5](https://schema.datacite.org/meta/kandidat/) | `creators` / `contributors` / `publisher` / `relatedIdentifiers` shape for cross-catalog dataset attribution | `aidocs/frontend/100 §4.3` Collection footer | When `<SourceInstanceFooter>` renders cross-instance attribution, the natural export target is a DataCite record — the source instance is the original `publisher`, dest is `relatedIdentifiers[IsDerivedFrom]` | unread |
| 2026-05-23 | [RO-Crate 1.1](https://www.researchobject.org/ro-crate/1.1/) | Research Object Crate metadata format — `wasDerivedFrom` chains for cross-repository data movement | `aidocs/frontend/100 §6` export hook | Eventual export target for the prov projection — the aggregator response is already RO-Crate-shaped if we serialize JSON-LD with the right context | unread |
| 2026-05-23 | [DIN EN 9100:2018 §7.5 + §8.4](https://www.beuth.de/de/norm/din-en-9100/293898894) | Documented information control + control of externally-provided products and services | `aidocs/frontend/100 §4.4` audit pane | EN 9100 §7.5 (documented information) and §8.4 (externally-provided processes) are the audit clauses the permissions-snapshot pane targets — an auditor verifying "who could see this when it was imported from cube3" | unread |
...
| RL-FAIR4-RDA-CHARTER | Read the full RDA FAIR4ML IG charter PDF (`https://www.rd-alliance.org/wp-content/uploads/2024/05/FAIR4ML-RDA-IG-Charter.pdf`); fetch failed in the 2026-05-23 survey because the PDF is unreadable through the fetch path. Document the IG's stated scope + roadmap + governance shape — bridges to FAIR4-PUB-VENUE (joint-paper feasibility with Castro et al.) | The fork's `aidocs/semantics/fair4ml-fair4ai-fai2r-2026-05-23.md §1.1` cites the IG but couldn't grep the charter directly; due-diligence gap before approaching the IG | open |
| RL-FAIR4-HUERTA-INITIATIVES | Read end-to-end the eight FAIR-for-AI initiatives Huerta 2023 enumerates (FAIR4HEP, ENDURABLE, NIH Common Fund Data Ecosystem, BioDataCatalyst, Garden, Braid, HPC-FAIR, FAIR Surrogate Benchmarks Initiative) for predicate vocabularies that f(ai)²r might align with or learn from | Huerta 2023 catalogues but doesn't deeply analyze; each initiative may have proposed predicates that compete with or complement f(ai)²r `AuthoringPass`/`Prompt`/`verificationState` | unread |
| RL-FAIR4-MUKHERJEE-2025 | Read end-to-end the 2025 healthcare FAIR-AI framework (Mukherjee et al., npj Digital Medicine, doi:10.1038/s41746-025-01900-y). It's a clinical-AI deployment-evaluation framework distinct from the RDA FAIR4ML vocabulary; check whether its pre/post-implementation checklist is mappable to f(ai)²r predicates | The 2026-05-23 survey notes it exists but defers detailed reading; understanding the deployment-track FAIR-AI work matters for the §5 EU AI Act mapping | unread |
| RL-FAIR4-MLENTORY | Read MLentory architecture + adopted FAIR4ML profile (NFDI4DataScience service; one of the only two production adopters). Understand: which FAIR4ML predicates are populated automatically vs left empty, what the harmonization pipeline looks like across HuggingFace + OpenML, and whether MLentory is open-source so the fork could reuse it | The fork's `shepard-plugin-publisher` design needs a reference implementation; MLentory is the closest fit | unread |
| RL-FAIR4-CROISSANT-NEXT | Track Croissant v2 + the FAIR4ML/Croissant convergence work (per 2024 ODI blog post, the two communities are coordinating). The fork's FAIR4-CROISSANT-EXPORT row depends on the substrate stabilising | The MLCommons working-group page lists planned next-version features; need to know what's in v2 before designing the export pipeline | unread |
| RL-FAIR4-OAI-LLM-TRACK | Read the OAEI 2024 LLM track results + OntoAligner Apache-2.0 evaluation (per ONT-AI-MAP1 outcome). Understand how LLM-based ontology alignment performs on RDA-flavour vocabularies (schema.org extensions like FAIR4ML, Croissant) — the alignment between f(ai)²r predicates and FAIR4ML predicates in FAIR4-BRIDGE-ONTOLOGY may benefit | The §5.2 bridge ontology is hand-authored today; if LLM-alignment is reliable enough, this becomes automatable | unread |
