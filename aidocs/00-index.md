...
| 83 | [`83-pointcloud-and-live-overlay.md`](83-pointcloud-and-live-overlay.md) | **Point Cloud Integration and Live Overlay Modalities**. Extends `shepard-plugin-cad` (aidocs/78) with a `PointCloudReference` payload kind (PLY, E57, LAS/LAZ; Potree tile streaming; CAD–cloud ICP alignment via Open3D sidecar; deviation map auto-HEATMAP). Explains why point clouds belong with CAD (engineering geometry + alignment workflow) rather than with the PostGIS `SpatialDataReference` (geographic data). Extends SB1 (aidocs/81) with six additional live overlay modalities: CAMERA_FRUSTUM + PiP video (SB2a), AE event cloud (SB2b), raster projection of thermography/DIC images (SB2c), multi-channel synchronised timeline (SB2d), nominal-vs-actual trajectory comparison (SB2e), FEM frame animation (SB2f). Unified viewer capabilities matrix. PC1a–PC1d + SB2a–SB2f phasing. | Design |
