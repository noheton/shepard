...
| 43 | [`43-ai-opportunities.md`](43-ai-opportunities.md) | AI opportunities survey — split between traditional ML (anomaly detection, channel-quality scoring, forecasting, outlier detection, similarity search, learn-to-rank) and LLM integration (natural-language search, lab-journal authoring assist, auto-summarisation, semantic-annotation suggestion, RO-Crate description generation, conversational lineage, notebook scaffolding). Five hard architectural constraints (data residency / cost / auditability / human-in-the-loop / reproducibility); AI1 series with 16 sub-IDs (AI1a–AI1p). Local-model default; hosted-model bridge is opt-in only. Recommended phase order AI1a → AI1b → AI1c → AI1d → AI1e → AI1g → AI1f → AI1h → AI1i. shepard-AI sidecar mirrors the HSDS sidecar pattern from `aidocs/35` | Survey |
