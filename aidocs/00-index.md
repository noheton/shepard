...
- [Chapter H. Strategy, landscape & stakeholders](#chapter-h--strategy-landscape--stakeholders) — `70`–`82`
...
| 78 | [`78-cad-geometry-annotator.md`](78-cad-geometry-annotator.md) | **3D Geometry & FEM Annotator (`shepard-plugin-cad`)**. `CadReference` payload kind. Nine-row format matrix: native glTF, STL/OBJ, STEP/IGES (cascadio WASM + pythonocc sidecar), URDF bundles, VTK/FEM (vtk.js + meshio), NX/CATIA via STEP, DLR-internal Paradigms kernel (non-public, `CadConverter` SPI placeholder), Pandora converter (DLR-BT-SIN, non-public). Dual renderer (Three.js for CAD, vtk.js for FEM). Five annotation interaction modes (point, region, surface, FEM node/element set, URDF link). `GeometryAnnotation` Neo4j model with dual-encoding (world-space + topology-relative). Scene graph export API (glTF, USD, SDF, VTU) with annotation embedding. SSE annotation stream for live simulator sync. CAD1a–CAD1j phasing. | Design |
| 79 | [`79-cpacs-annotator.md`](79-cpacs-annotator.md) | **CPACS Annotator (`shepard-plugin-cpacs`)**. CPACS (Common Parametric Aircraft Configuration Schema) is DLR's parametric aircraft configuration format — geometry is generated by TiGL, not stored as a mesh. `CpacReference` PayloadKind + TiGL sidecar Docker service (CPACS XML → GLB via pythonocc). `:CpacComponent` Neo4j nodes mirror the CPACS component tree (fuselage, wings, engines; `uID`-keyed). `CpacAnnotation` anchored by CPACS XPath — more stable than face indices across design iterations. `needsReview` flag when re-upload changes a component's mesh hash. Non-geometry CPACS data (aero tables, mass breakdown) surfaced via XPath query endpoint. RCE → CPACS provenance chain (gated on aidocs/80 RCE1b). CPACS1a–CPACS1f phasing. | Design |
| 80 | [`80-rce-integration.md`](80-rce-integration.md) | **RCE Integration with Provenance Tracking (`shepard-plugin-rce`)**. Deep integration with DLR's Remote Component Environment (RCE) workflow tool — closes gaps in the existing `RCE2Shepard` adapter (Dressel et al., DGLR 2022). Three roles: shepard sink (RCE components write outputs to shepard as DataObjects), shepard source (components read inputs from shepard), and provenance tracking. `Shepard Source` + `Shepard Sink` RCE tool integrations (JSON descriptors shipped as plugin JAR resources). OpenLineage emission per component execution → `:Activity` PROV-O nodes in Neo4j. CPACS-typed provenance chain for MDO workflows. MDO data bus: parameter-sweep Collections, batch ingest API. `:RceConfig` admin singleton + full CLI parity. Migration path from `RCE2Shepard`. RCE1a–RCE1f phasing. | Design |
| 81 | [`81-spatial-data-binding.md`](81-spatial-data-binding.md) | **Spatial Data Binding: Linking Geometry to Measurements**. Extends `GeometryAnnotation` (aidocs/78) with a `:DataBinding` Neo4j node that links spatial locations on 3D geometry to any shepard Reference (timeseries, files, images, structured data). Five display modes: BADGE (scalar label), HEATMAP (surface colour map), GLYPH (arrow/cone for vectors), JOINT_ANGLE (URDF joint animation from timeseries channel), DEFORMATION (FEM node displacement from a structured-data frame). Time scrubber for historical playback; SSE live mode pins to the latest timeseries sample. ZLP-specific data chains (AFP layup → thermographic inspection, welding → thermocouple → tensile shear, robotic testing → joint angle → force). Semantic relationship shortcut types (`[:CHARACTERISES]`, `[:VALIDATES]`, `[:INSPECTS]`, `[:PRODUCED_BY]`, `[:COMPANION_TO]`, `[:LOCATES_SENSOR]`). SB1a–SB1f phasing. | Design |
| 82 | [`82-zlp-augsburg-stakeholder.md`](82-zlp-augsburg-stakeholder.md) | **ZLP Augsburg stakeholder brief** — DLR's Zentrum für Leichtbauproduktionstechnologie, shepard's home institute and primary developer. Not a pitch (they already deploy); instead documents full data landscape across five research campaigns (AFP, resistance welding, NDT/QA, material characterisation, robotic component testing), maps new capabilities (CAD annotator, CPACS annotator, RCE integration, spatial data binding) to each campaign, identifies six honest gaps (CT voxel, DIC proprietary formats, control-loop latency, large-assembly STEP, weld recipe versioning), and proposes ZLP as the reference deployment that other DLR institutes point to. Feature-maturity ladder; concrete four-point ask. | Stakeholder |
| 83 | [`83-pointcloud-and-live-overlay.md`](83-pointcloud-and-live-overlay.md) | **Point Cloud Integration and Live Overlay Modalities**. `PointCloudReference` (PLY/E57/LAS/LAZ; Potree streaming); CAD–cloud ICP alignment (Open3D); deviation map HEATMAP (PC1c); surface-annotation matching with per-feature deviation stats and tolerance pass/fail (PC1e); live streaming (PC1d). Eight additional overlay modes beyond SB1: SB2a–SB2h (camera frustum + PiP, AE event cloud, raster projection, multi-channel timeline, nominal-vs-actual trajectory, FEM frame animation, ultrasonic C-scan + A-scan panel, POSE_6D 6D pose triad). SA1 Spatial Analyzer integration (ground-truth coordinate frame, target-constrained alignment, T-Scan, 6D TCP tracking). RDK1 RoboDK integration (URDF chain, SA→RoboDK→shepard calibration provenance). IL1 managed ingest pipelines (IngestTrigger + FormatParser SPIs; UltrasonicOmniParser as priority first parser). | Design |
| 84 | [`84-live-digital-twin.md`](84-live-digital-twin.md) | **Live Digital Twin: Moving Objects, Production Cell Scene, and State Streaming**. Moving-frame `GeometryAnnotation` (`urdfLinkId` + `localPos*` so annotations track with robot links). `DigitalTwinScene` composite entity (multiple URDF/mesh members, declared static transforms, dynamic workpiece transforms). WebSocket live state stream (`/v2/digital-twin-scenes/{appId}/live`): CBOR StatePacket with delta encoding (joint angles, sensor values, poses, events); I-frame every 10 s. Optional `shepard-sidecar-renderer` (headless Chromium + WebRTC video output, < 50 ms latency, interactive via WebRTC data channel back-channel for camera navigation). Historical replay via `scrubTo` message (same StatePacket format — same viewer code). `DigitalTwinSnapshot` entity (PNG + state timestamp + view config; manual, alarm-triggered, or cycle-end automatic capture; alarm-triggered snapshots fire when DataBinding threshold is crossed). DT1a–DT1g phasing. | Design |
