...
| 49 | [`49-in-app-user-docs.md`](49-in-app-user-docs.md) | **In-app user docs**: shepard frontend serves `/help` from the same `docs/*.md` source the Pages site builds, so a casual user gets help without leaving shepard. **Playwright screenshot pipeline** captures against the maintainer's test deployment `shepard.nuclide.systems` (`aidocs/40 §3c`); PNGs commit back to `docs/assets/screenshots/` so the docs always match the running UI. Marker convention (`<figure data-target="..." />`) already in place in `docs/showcase.md`. **Version stamping** ties in-app docs to the running backend version. D1a–D1g phasing; D1c adds task-shaped help pages (upload-data / share-collection / export-rocrate / process-step) | Design done |
