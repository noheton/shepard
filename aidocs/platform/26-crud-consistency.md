...
**Legend:** ✓ = present, ✗ = missing. "Paginated" per `aidocs/archive/18-pagination-inventory.md`. Counts in parentheses denote multiplicity (e.g., "✓ POST (3×)" = three POST endpoints).
...
**Citation:** `aidocs/platform/23-api-critique.md` §2.1 (timeseries composite keys identified as design anomaly).
...
**Citation:** `aidocs/platform/23-api-critique.md` §3 identifies this as "per-kind annotation rest fragmentation" — 4 per-kind rests (Per-*-Rest, authz, permissions, metrics) creating 4 scaffold clones. **Recommendation:** A2 (monolithic Rest decomposition) from `aidocs/16-dispatcher-backlog.md` would consolidate these.
...
### 5. **Resolve Timeseries composite-key pattern to single-ID or decomposed path** (P5)
   ...
   - **Citation:** `aidocs/platform/23-api-critique.md` §2.1.
...
- **Pagination:** 38 list endpoints identified in `aidocs/archive/18-pagination-inventory.md` (reuse count; not re-inventoried here).
- **Existing critique:** `aidocs/platform/23-api-critique.md` already catalogs PATCH/PUT confusion, timeseries composite keys, and annotation redundancy — this doc tabulates scope and adds actionable metrics.
