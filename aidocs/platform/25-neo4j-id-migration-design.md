...
**Status.** Phase 1 (L2a) **landed** `fec7979` (PR #1003). Phase 2 (L2b) **landed** `796bc11` (PR #1020). C5 (Cypher-injection fix) **landed** `e90bfd8` (PR #1021); C5b (second-wave injection sites in `*ReferenceDAO` / `GenericDAO` / `VersionDAO` / `SemanticAnnotationDAO`) **landed** `c707e56`. **Phase 3 (L2c) landed (this branch)** — `EntityIdResolver` `@RequestScoped` bean translates the OGM `Long` ↔ `appId` at the DAO boundary; 14 DAO files flipped from `WHERE ID(e) = $entityId` (`long`) to `WHERE e.appId = $appId` (`String`); public DAO method signatures stay `long` for caller-compat; permissions cache key stays `long` (per §3.3); `Neo4jQueryBuilder`'s search-JSON-derived numeric-id predicates and `PermissionsService.isAllowed` path-segment dispatch **deliberately not touched** (those land in L2d). L2d still gated on P4 + H4.

**Snapshot date.** 2026-05-07.

**Companion docs.** Reads as a fourth member of the C-section (`12`/`13`/`14`); extends `aidocs/data/12-timescaledb-performance-analysis.md` §11 (identifier discipline) — does not duplicate it.
...
- **Hibernate / OGM ergonomics.** `java.util.UUID` round-trips through Neo4j-OGM and the Neo4j Java driver as a `STRING` property without custom converters. The migration to a future driver-only stack (when Neo4j-OGM is finally retired — `aidocs/archive/03-issues-status.md` #274) keeps the type stable. v7 vs v4 is a generation-side decision; storage and retrieval are identical.
...
4. `AbstractEntity.id: Long` (the OGM mirror) can be removed *only* once Neo4j-OGM is gone (`aidocs/archive/03-issues-status.md` #274). Until then, the field stays but no Cypher reads it externally.
