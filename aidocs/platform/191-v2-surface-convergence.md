...
| `v2.svdx.*` | `plugins/fileformat-svdx` | ✓ **DISSOLVED 2026-06-12** (V2CONV-A7-SVDX-REST-DISSOLVE): `fileKind=svdx` → `FileFormatPlugin` (annotate `.svdx` on upload) **+** the real CSV→TimescaleDB ingest rehomed as `SvdxCsvTransformExecutor` (`SvdxCsvIngestShape` `MAPPING_RECIPE`, via `POST /v2/mappings/{appId}/materialize`); bespoke `/v2/svdx/*` deleted. Mirrors KRL/URScript. |
...
| `/v2/svdx/ingest` (`SvdxIngestRest`, fileformat-svdx) | **✓ DISSOLVED 2026-06-12** | The survey correctly found this is a real CSV→TimescaleDB ingest (parses TwinCAT `.csv`, writes channels via `TimeseriesService`), NOT an upload side-effect. Resolution (V2CONV-A7-SVDX-REST-DISSOLVE): the capability is **rehomed** onto the generic MAPPING_RECIPE seam — `SvdxCsvTransformExecutor` claims `SvdxCsvIngestShape` and reuses `SvdxCsvIngestionService` verbatim, materialized via `POST /v2/mappings/{appId}/materialize`. The bespoke `/v2/svdx` namespace + its IO shapes are deleted; the deterministic idempotency contract is preserved. Mirrors KRL (B5) / URScript. No remaining non-Tier-3 exception — the generic-surface equivalent now exists. |
