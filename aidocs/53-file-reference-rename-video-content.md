...
| ID | Slice | Size | Gate |
|---|---|---|---|
| **VID1a** | Fully-uploaded MP4 on the FileBundle path with a **`VideoStream` wrapper entity** stored as a thin Neo4j-side handle pointing at one ShepardFile. `POST /v2/videos` (single-shot upload). `GET /v2/videos/{id}/playlist.m3u8` derives a one-segment manifest. No live ingest, no frame extract. Permission gating via existing path. | M | None (rides FileBundle infra; if §1 ships first the path is straight). |
