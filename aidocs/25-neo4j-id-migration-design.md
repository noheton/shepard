...
**Status.** Phase 1 (L2a) **landed** `fec7979` (PR #1003). Phase 2 (L2b) **landed** `796bc11` (PR #1020). C5 (Cypher-injection fix) **landed** `e90bfd8` (PR #1021); C5b (second-wave injection sites in `*ReferenceDAO` / `GenericDAO` / `VersionDAO` / `SemanticAnnotationDAO`) **landed** `c707e56`. **L2c is fully unblocked** — every previously-unsafe `WHERE ID(e) = …` site already binds a named long parameter; the parameter-type swap from `long` to `String` for `appId` is mechanical from here. L2d gated on P4 + H4.
