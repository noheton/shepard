...
* **Issues found**: **23 distinct issues** across CRITICAL / MAJOR /
  ...
  `19`). The backend's `/v2/collections` endpoint also returns 404 to
  all requests — separate `BUG-V2-COLL-LIST-404`.
...
| I2 | `BUG-V2-COLL-LIST-404` | (API) `/shepard/api/v2/collections` | **CRITICAL** | `GET /v2/collections` and `GET /v2/collections?appId=...` both return 404 "Resource not found". `GET /v2/collections/{appId}` (by literal UUID) also returns 404. The `/v1/collections/{long-id}` works. Confirmed via curl with valid alice JWT. | `curl -H "Authorization: Bearer $TOK" "https://shepard.nuclide.systems/shepard/api/v2/collections?size=5"` → 404. | Backend follow-up — L2d gate. The frontend's `BUG-COLL-APPID-ROUTE-001` fix cannot land without this. |
...
| ID | what it gives the user | acceptance | size | priority | risks | links |
|---|---|---|---|---|---|---|
| BUG-COLL-APPID-ROUTE-001 | URLs `/collections/{appId}` work end-to-end | `parseCollectionId` branches on UUID-shape; v2 endpoint hit for appId, v1 for numeric. Vitest covers both paths. | S | **CRITICAL** | gated on BUG-V2-COLL-LIST-404 backend fix | this file §Issue catalogue I1 |
| BUG-V2-COLL-LIST-404 | `GET /v2/collections` returns the collection list | endpoint registered + visible to JAX-RS; integration test covers list + by-appId paths | M | **CRITICAL** | none | this file §Issue catalogue I2 |
