...
| W7b  | **TPS raw-data line-scan → brush-trace** (operator correction 2026-06-02: each PNG row = one sensor measurement instant along the track, NOT process video) | `Track_NN__Run_NN_/files/TPS raw data.0…37` | one `SpatialDataContainer` of `kind=brush-trace` per track (plugin `spatiotemporal` v6) | extend `shepard-plugin-spatial-importer` with line-scan-PNG decoder + Y-axis→time mapping + value-vector projection | 8,251 tracks × 38 chunks × 964 rows = **313 M measurement points** (or aggregated to per-chunk profiles) | wave 7 (pointclouds) done; line-scan extension shipped | **gap-blocked on MFFD-SPATIAL-LINESCAN-IMPORTER-1** |
