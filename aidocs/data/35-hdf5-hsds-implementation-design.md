...
(`aidocs/platform/20-epic-roadmap.md:302-381`) and demand-gauge §3.6
(`aidocs/archive/21-user-interest-gauge.md:68-78`). Picks one architecture
...
- **Storage migration mid-life.** POSIX → S3 migration is non-trivial.
  ...
  `aidocs/ops/22-admin-cli-draft.md` and triggers HSDS's offline export →
  re-import workflow.
...
- `aidocs/platform/20-epic-roadmap.md` §E7 — epic-level scope and the hard
  constraint that motivates this design.
- `aidocs/archive/21-user-interest-gauge.md` §3.6 — demand sizing
  ("low-medium," one named asker).
- `aidocs/16-dispatcher-backlog.md` — A5 series queueing entry will
  follow this design landing.
- `aidocs/platform/25-neo4j-id-migration-design.md` — L2c gates A5a so HDF
