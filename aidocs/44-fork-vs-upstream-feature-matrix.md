...
| **HMC Kernel Information Profile baseline** (`Minter` SPI in core, `MockMinter` default impl, `MinterRegistry` discovery + fail-fast; `:Publication` entity + `HAS_PUBLICATION` edge; `POST /v2/{kind}/{appId}/publish` with Writer/Manager auth + idempotent re-POST + `?force=true` re-mint; unauthenticated `GET /v2/.well-known/kip/{pid-suffix}` returning the public HMC KIP JSON-LD record; `PublishableKindRegistry` for URL-segment dispatch — adding bundles/files/lab-journal-entries doesn't change the URL shape; V29 migration; **KIP1g (resolver as plugin) shipped** — `KipResolverRest` + `KipRecordIO` extracted into `shepard-plugin-kip` per CLAUDE.md plugin-first heuristic #2, wire shape byte-identical to pre-KIP1g; ePIC + DataCite plugin shapes designed for KIP1c/d per ADR-0023 drop-in-JAR shape) | n/a | **KIP1a + KIP1g shipped** — KIP1a baseline (61 tests, 94.5% line / 80.4% branch on `de.dlr.shepard.publish.*` + `de.dlr.shepard.v2.publish.*`; KIP1b folded in). KIP1g (this PR) extracts the HMC-flavoured resolver + record JSON-LD shape into a new `plugins/kip/` module (`shepard-plugin-kip`); the `Minter` SPI, `:Publication` entity, `PublicationDAO`, `PublishableKindRegistry`, generic `POST .../publish` orchestration and `PublicationIO` wire shape stay in core. Endpoint path + JSON-LD body + RFC 7807 problem responses byte-identical to pre-KIP1g. The 6 `KipResolverRestTest` cases moved with the resource; the `with-plugins` Maven profile now declares both `shepard-plugin-unhide` + `shepard-plugin-kip`. | **✓ ↑** (KIP1a + KIP1g, this commit) | `aidocs/66` / `aidocs/16 KIP1a` + `KIP1g` |
