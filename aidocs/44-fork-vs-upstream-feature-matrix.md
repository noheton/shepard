...
| **S3-compatible adapter plugin** (`shepard-plugin-storage-s3`) — any S3-compatible endpoint (AWS S3, Cloudflare R2, Backblaze B2, Wasabi, Garage, SeaweedFS, Ceph RGW, MinIO); slots into the FS1a SPI without touching `FileContainerService`; five admin REST endpoints + `shepard-admin storage s3 ...` CLI parity; `:S3StorageConfig` singleton + AES-GCM-256 credential at rest; V35 migration | none | shipped (FS1b) | **✓ ↑** | `aidocs/45 §3.2` + `aidocs/63 ADR-0024` + `docs/reference/storage-s3.md` |
