...
| **DBpedia Databus rich-reference plugin** (preview / description / title fetched + 24h-cached; `:DbpediaDatabusReference` typed reference + `:DbpediaDatabusConfig` admin-configurable singleton; admin REST + reference CRUD REST + `shepard-admin references dbpedia-databus ...` CLI parity; JSON-LD metadata fetch from Databus artifact URIs with 24h cache TTL + OAuth CC support for private instances; `shepard-plugin-reference-dbpedia-databus` drop-in JAR; V33 + V34 migrations; off-by-default `enabled=false`) | n/a | **REF1c shipped** — `shepard-plugin-reference-dbpedia-databus` module: `DatabusHttpClient` (JDK `HttpClient`, JSON-LD parsing, OAuth CC token cache, 1 retry on 5xx/IOException), `DbpediaDatabusConfigService` (seed-on-first-start, RFC 7396 merge-patch, AES-GCM credential encrypt/fingerprint), `DbpediaDatabusReferenceService` (preview, stale/fresh cache, `validateUri`), admin REST + reference REST + CLI. 90 unit tests (8 test classes). Operator runbook: `docs/reference/reference-dbpedia-databus.md`. | **✓ shipped** (REF1c) | `aidocs/58 §7` + `aidocs/16 REF1c` |
