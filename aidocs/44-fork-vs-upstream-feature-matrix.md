...
| **Deployable demo instance (`make demo-up`)** — one-command bootstrap that deploys a complete, seeded shepard at <http://localhost:3000/> in well under 90 seconds. Layered compose (`infrastructure-local/docker-compose.yml` + `docker-compose.demo.yml` overlay); one-shot `demo-seeder` service that waits for the backend's bootstrap token, consumes it to promote a Keycloak `admin` user to instance-admin, mints a deterministic admin API key, then applies six idempotent Cypher seed files (users / 5 Collections / 12 DataObjects / 3 FileContainers / 2 references / 1 KIP-resolvable LocalMinter Publication). Pre-imported Keycloak `shepard-demo` realm with four test users (alice / bob / admin / harvester, passwords `<user>-demo`). Eight `make` targets (`demo-up` / `demo-down` / `demo-reset` / `demo-seed` / `demo-status` / `demo-logs` / `demo-smoke` / `help`). Operator runbook at `infrastructure-local/README-demo.md`, public-docs front door at `docs/reference/deployment-quickstart.md`, top-level README "Try it locally" section. Default-safe posture: minter=local (no external service calls), `unhide.enabled=false` (operator-opt-in), DataCite gated on operator opt-in. All credentials obviously-fake; image-size budget honoured (seed-files <5 MB). | none | shipped — `infrastructure-local/docker-compose.demo.yml`, `infrastructure-local/demo-seed/{run-seeder.sh, cypher/*.cypher, keycloak/shepard-demo-realm.json, seed-files/*}`, `infrastructure-local/.env.demo`, `infrastructure-local/wait-for-backend.sh`, `infrastructure-local/demo-smoke-test.sh`, `infrastructure-local/README-demo.md`, repo-root `Makefile`, `docs/reference/deployment-quickstart.md`, README "Try it locally" section. Compose merge validates via `docker compose config --quiet`; bash scripts shellcheck-clean. Caveat: file *blobs* not seeded into GridFS (graph metadata only — Download returns 404; FS1a / DX5b will close). | **✓ ↑** (DX5a, this commit) | `aidocs/16 DX5a` |
| Live-reload contributor dev loop (`make dev`) — backend in Quarkus dev-mode + frontend in Nuxt dev-mode atop the DX5a seeded infra | none | TBD | 📐 (queued, DX5b) | `aidocs/16 DX5b` |
| CI demo-smoke workflow — runs `make demo-up` + `make demo-smoke` on every push to `main` to catch deployment-config rot | none | TBD | 📐 (queued, DX5c) | `aidocs/16 DX5c` |
