...
| Storage-backend plugin SPI (`PayloadKind` + `PayloadStorage`); new payload kinds drop in as plugins | none | TBD | 📐 (queued, PL1a) — built on top of PM1a's PluginManifest SPI | `aidocs/47 §2` |
...
| **Entity versioning (Collection / DataObject)** (`:EntityVersion` Neo4j entity attached via `HAS_ENTITY_VERSION` to Collection / DataObject — distinct from the legacy `:Version`-backed `has_version` edge from the pre-ENT1 versioning model; `HasAppId` (UUID v7) on every version row; fields `versionLabel` / `versionOrdinal` / `createdAt` / `createdBy` / `parentEntityKind` / `parentEntityAppId` / `note`; per-version `:Permissions` child — one ACL per version, inheriting the previous version's ACL on create then evolving independently; REST surface under `/v2/{kind}/{appId}/versions/...` with `POST` (create with optional `{label, note}`; server suggests `v<n+1>` when label is blank, user labels validated against `[a-zA-Z0-9][a-zA-Z0-9.\\-+]{0,63}` and rejected if purely numeric) / `GET` (list, filtered by per-version Reader ACL, ordered by `versionOrdinal DESC`) / `GET /{label}` (single, gated on Reader) / `PATCH /{label}/permissions` (per-version ACL, gated on Manager) / `DELETE /{label}` (gated on Manager, refuses the last remaining version); RFC 7807 envelopes `versions.not-found` / `versions.label.duplicate` / `versions.label.invalid` / `versions.cannot-delete-only` / `versions.kind.unsupported`; V35 migration with `appId_unique_EntityVersion` + `parentAppId_label_unique_EntityVersion` constraints + v1 backfill for every existing Collection + DataObject deep-cloning the parent's `:Permissions` graph; CDI `VersionCreatedEvent` for ENT1b–e observers) | n/a | **ENT1a shipped** — graph + REST baseline of `aidocs/16` ENT1 umbrella. KIP1h's `Publication.versionNumber` and ENT1a's `EntityVersion.versionLabel` are separate concepts in ENT1a; ENT1c reconciles them. File-payload Copy-on-Write per version is deferred to ENT1b (gated on FS1a's `FileStorage` SPI). Publish hookup deferred to ENT1c. Vue selector dropdown deferred to ENT1d. Cross-cardinality (Bundle / File / Reference) deferred to ENT1e. Tests: 27 `EntityVersionDAOTest` (Mockito on Neo4j session) + 27 `EntityVersionServiceTest` (happy-path + duplicate-label + invalid-label + overlong-note + manager-required + last-version-delete-refusal + ACL-inheritance + owner-transfer + label-shape-validation cases) + 23 `EntityVersionRestTest` (auth gates + RFC 7807 envelopes + happy paths for all five endpoints); 77 new total. JaCoCo on new code: `de.dlr.shepard.versioning` 82% line / 70% branch; `de.dlr.shepard.v2.versioning.resources` 91% line / 78% branch; `de.dlr.shepard.v2.versioning.io` 95% line / 62% branch — all above the 70% new-code gate. SpotBugs clean. Backend test count: 2350 → 2427 (+77). | **✓ ↑** (ENT1a, this PR) | `aidocs/16 ENT1a` |
