...
| **PROV-O provenance + activity capture + dashboard** (`:Activity` Neo4j entity, JAX-RS request filter, `GET /v2/provenance/{activities,count,stats,entity/{appId}}` query endpoints, per-Collection activity sparkline UI) | n/a | **PROV1a/b/c/d/f/g shipped** — capture filter, query endpoints, stats aggregation with cumulative integral, retention TTL, PROV-N JSON content negotiation, and the per-Collection Vue dashboard (sparkline + cumulative overlay + action-kind histogram + time-range picker). Instance-admin dashboard (PROV1e) queued. | **✓ ↑** | `aidocs/55` / PROV1a-c commits + PROV1d (this PR) |
...
| **OpenAPI client-generator pick** (Kiota for `/v2/`, OpenAPI Generator retained for byte-frozen `/shepard/api/...`, Hey API as TS-only tactical secondary) | n/a | **CG1a shipped** (Kiota baseline wired for Python + TypeScript + Java under `clients-v2/`; CI workflow `.github/workflows/clients-kiota.yml` regenerates on every release tag); CG1b (OpenAPI Generator legacy maintenance) queued; CG1c (PyPI / npm / Maven Central publishing) queued; CG1d (Go / Rust / C#) deferred; CG1e (Hey API frontend) deferred | **✓ ↑** (CG1a, this commit) | `aidocs/57` / `aidocs/63` ADR-0022 (sibling PR) |
...
| **RO (Relation Ontology) added to the pre-seed bundle** (`obo-relations.ttl`, SHA-256 pinned, CC0 licence, LUMEN seed uses RO terms post-ONT1b) | n/a | ONT1a shipped (bundle + manifest + tests); ONT1b (LUMEN seed cites RO terms) + ONT1c (frontend ontology-picker) queued | **✓ ↑** (ONT1a, this commit) | `aidocs/58 §6` + `aidocs/48` |
