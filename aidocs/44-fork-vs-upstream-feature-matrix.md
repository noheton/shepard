...
| **Point Cloud + Live Overlay** (PC1 + SB2) — `PointCloudReference` PayloadKind (PLY/E57/LAS, Potree streaming); CAD–cloud ICP alignment (Open3D sidecar); deviation map HEATMAP; live streaming (PC1d); six additional overlay modes: CAMERA_FRUSTUM+PiP, AE event cloud, raster projection, multi-channel timeline, nominal-vs-actual trajectory, FEM frame animation | n/a | design done | 📐 (queued, PC1a; gated on CAD1a) | `aidocs/83` |
