...
- **Status**: **DONE**. RFC 7807 (`application/problem+json`) is now the default response shape. Known shepard exceptions surface their `getMessage()` as the `detail` field; the unknown-5xx path returns a generic `"An internal server error occurred. Reference: <traceId>"` so Hibernate / Neo4j / Mongo internals never reach the wire. Legacy `ApiError` shape preserved for clients that explicitly request `application/json` (per the API-version policy in `CLAUDE.md`). New `ShepardErrorCode` catalogue (10 entries: `auth.expired`, `auth.invalid_token`, `auth.unauthenticated`, `auth.forbidden`, `validation.body`, `validation.field`, `permission.denied`, `not_found.entity`, `conflict.entity`, `internal.unexpected`). New `ProblemJson` record with `Map<String, Object> extensions` for per-code extras (Jackson `@JsonAnyGetter` flattens into the body). New `TraceIdProvider` (`@RequestScoped`, UUID-per-request) used as the `instance` field; honours an inbound `X-Request-Id` header when present. **Subsumes M7**: full stacktrace logged at `debug`; the `error` line carries only `traceId + exception class + http method + request path` — no request body, no headers (auth / api-key / cookie), no exception message. New tests: 20 in `ShepardExceptionMapperTest` (5xx leak suppression, log-level capture, content negotiation, trace-id sourcing, code mapping for 401/403/404/400/409); 2 in `TraceIdProviderTest`.
- **Original location**: ~~`backend/src/main/java/de/dlr/shepard/common/exceptions/ShepardExceptionMapper.java:21-24`~~
- **Original recommendation**: ~~Return generic message for `INTERNAL_SERVER_ERROR`; only expose message for explicit `WebApplicationException` subclasses; log full stack only at `debug` or with sanitization.~~
- **Fix effort**: S
- **Tracked**: No
...
| M7 | ~~`ShepardExceptionMapper.java:21`~~ | ~~All exceptions logged with full stack at error — log-volume DoS plus PII~~ — **DONE** (subsumed by H4; full stack at `debug`, error log carries only `traceId + class + method + path`). | — | S |
