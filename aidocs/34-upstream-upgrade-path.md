...
| **C5** | Cypher injection fix — parameter-binding + property-name allowlist in `Neo4jQueryBuilder`, `PermissionsDAO`, `DataObjectDAO` | ZERO | No endpoint or config impact (internal Cypher construction only) | — | None. Search and DAO query shapes change internally — the wire contract (request body, response shape) is unchanged. Closes the injection vector against user-controlled property names + IRI values; subsumes M9. Backlog **C5b** tracks the analogous fix for the `*ReferenceDAO` family + `GenericDAO.getSearchForReachableReferences*` + `VersionDAO` + `SemanticAnnotationDAO` sites — same shape, separate PR for tight scoping. |
