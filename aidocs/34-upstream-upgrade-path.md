...
| **J1a — lab-journal render** | New `GET /v2/lab-journal/{appId}/render` endpoint (J1a, `aidocs/37`). Parses a LabJournalEntry's `content` field as CommonMark + GFM (tables + strikethrough) and returns sanitised `text/html`. New runtime deps: `org.commonmark:commonmark:0.22.0` + two GFM extensions. Sanitisation via existing jsoup (OWASP `Safelist.relaxed()`). | ZERO | New `/v2/` endpoint — opt-in, no upstream client impact. No new config keys. No schema change. Permission model unchanged: requires Read on the owning Collection (same gate as the existing `GET /shepard/api/lab-journal-entries/{id}`). | — | **None.** Lab-journal entries already in Neo4j are rendered on-the-fly from the existing `content` field — no migration needed. The CommonMark parser is pure-Java in-process; no external service. |
| **U2a — GitCredential CRUD + PAT encryption** | New `:GitCredential` Neo4j label + `OWNS_CREDENTIAL` relationship from `:User`. New `/v2/me/git-credentials` CRUD (GET list, POST create, GET single, PATCH, DELETE). PAT stored AES-256-GCM encrypted; never returned on any response. New optional config key `shepard.secrets.encryption-key`. | **AWARE** | New optional config `shepard.secrets.encryption-key` (base64-encoded 32-byte AES-256 key). If absent → PAT storage disabled (`PATCH` for PAT update returns 501; all other credential operations work normally). If set to an invalid base64 or wrong-length value → **startup aborts** with a clear error. New `/v2/` endpoints only — zero impact on the frozen `/shepard/api/` surface. | `V20__Add_appId_constraint_GitCredential.cypher` (idempotent, single constraint add) | **To enable PAT storage:** generate a 32-byte key and base64-encode it: `openssl rand -base64 32`, then set `shepard.secrets.encryption-key=<output>` in your `.env` / `application.properties`. Without this key, credential metadata is stored but the PAT field is never written. Greenfield label — existing deployments with no Git credentials are unaffected. |
