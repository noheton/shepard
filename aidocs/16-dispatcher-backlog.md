...
|---|---|---|---|---|---|
| A0 | Instance-admin role mechanism: configurable `shepard.instance-admin.role`, populate `JWTPrincipal.roles` from configurable claim path (F8), mirror on API-key path, dual-source check (IdP OR `:HAS_ROLE` Neo4j edge), bootstrap-token mechanism, CLI grant/revoke/list/bootstrap. **Bundles C3 + F8.** | — | M-L | **design done** | `aidocs/51-instance-admin-role.md` is the implementation spec. Unblocks F1, A3b, P3c, L1 Phase 2. Single dispatch — when ready, lands as one slice. |
...
| H4 | Surface RFC 7807-shape error responses (existing high-finding from `07`) | `aidocs/07-security-issues.md` H4, `aidocs/19-architecture-feedback.md` §3 | M | **🚧 in flight** | Agent dispatched. Once landed, L2d's last gate clears. Bundles with M7 (sanitised exception logging). |
