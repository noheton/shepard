...
| PROV1c | On-demand stats aggregation + `GET /v2/provenance/stats?scope=instance\|collection\|user&id=&since=&until=` (totals + sparkline buckets + action-kind histogram in one payload; auto daily-/weekly-bucket switch at 90-day windows) | user request, `aidocs/55 §6` | M | **done** | 9 new tests (`ProvenanceStatsServiceTest`). v1 is per-request aggregation via new `ActivityDAO.aggregateBuckets` / `totalsByActionKind` / `distinctAgentCount` Cypher; pre-aggregated `:ActivityRollup` table deferred to PROV1c2 if perf demands it. Scope-instance gated `@RolesAllowed` admin; scope-user gated to self-or-admin; default window = last 90 days. |
