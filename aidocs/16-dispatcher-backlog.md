...
| R2b | File- and column-level selection on payloads (per-FileReference oid include list; per-TimeseriesReference column / time-window picks) | — | M | queued | Follow-up from R2 Phase 1; mirrors the dataship-side M4 (parked as X1) for the local payload model |
| R2c | Per-payload metadata-field redaction (e.g. include the annotation but redact its label) | — | M | queued | Follow-up from R2 Phase 1; opens a privacy / sharing surface |
...
| R2d2 | Emit per-entity subscriptions in the RO-Crate export — needs a URL-pattern → entity-URL matcher because subscriptions in shepard are URL-pattern-based, not per-entity-attached | — | M | queued | Architectural finding from R2d: `SubscriptionIO` is clean but the model doesn't have a per-entity subscription API; `SubscriptionFilter` matches request URLs against compiled patterns. R2d landed `addSubscriptionsFor(...)` on `ExportBuilder` for symmetry; the walker does not yet call it. The `metadata.subscriptions` boolean stays recorded-only until a synthesised-entity-URL helper is built. |
