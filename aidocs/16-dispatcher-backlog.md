...
| PROV1 | PROV-O provenance + activity dashboard | user request, `aidocs/55` | L | **design done** | `aidocs/55`. `:Activity` Neo4j entity (`HasAppId`); JAX-RS request filter on mutating endpoints; service-layer hook for non-REST flows; reads opt-in. Casual-user sparkline dashboard (per-Collection + instance-admin scope). PROV-N JSON export. 2-year retention default. PROV1a–PROV1g. |
