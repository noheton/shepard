...
| FB1 | FileReference → **FileBundle** rename + `FileGroup` sub-node grouping | user request, `aidocs/53` | M-L | **design done** | `aidocs/53`. Internal rename + `/v2/`-surface only; upstream wire shape stays frozen. `FileGroup` between `FileBundle` and `ShepardFile` carries grouping metadata (camera cyclic-capture use case). FB1a–FB1e. |
...
| T1 (revised) | Templates as first-class admin entity | user request, `aidocs/54` (supersedes `aidocs/39`) | L | **design done** | `aidocs/54`. `:ShepardTemplate` Neo4j entity (admin-only subgraph) replacing the `__templates` hack. Body = JSON DSL (inert at rest); versioning = copy-on-write; `:USES_TEMPLATE` + `:ALLOWS_TEMPLATE` edges; `/v2/templates/...` REST surface, all admin endpoints gated `@RolesAllowed("instance-admin")`. V15 migration. Recommended order: **CLI first, admin page second**. T1a–T1g. |
...
| PROV1b | Per-entity provenance trail — `GET /v2/provenance/entity/{appId}` (history of mutations on one entity) + `:USED`/`:GENERATED` edges from `:Activity` to target | user request, `aidocs/55 §6` | M | queued | gated on PROV1a (shipped). |
| PROV1c | Pre-aggregated rollup job (`:ActivityRollup` daily/weekly buckets) + `GET /v2/provenance/stats?scope=collection&id=&window=` | user request, `aidocs/55 §6` | M | queued | sub-second dashboard query path. |
...
| PROV1f | Nightly retention TTL job (`shepard.provenance.retention-days=730`) | user request, `aidocs/55 §7` | S | queued | gated on PROV1a (shipped). |
| PROV1g | PROV-N JSON export (`Accept: application/prov+json`) | user request, `aidocs/55 §6` | S | queued | gated on PROV1a (shipped). |
...
| V2S1 | v2 API simplification + output profiles + MCP-friendly OpenAPI | user request, `aidocs/56` | M-L | **design done** | `aidocs/56`. (a) Flat `/v2/dataobjects/{appId}` single-entity paths; bulk listings stay nested. (b) `?profile=metadata\|relations\|all` via Jackson views. (c) `x-mcp-tool-name` + `x-mcp-side-effects` OpenAPI extensions per operation. ArchUnit fence: `admin` side-effects ↔ `@RolesAllowed("instance-admin")`. V2S1a–V2S1f. |
...
| CP1 (was D1 in `aidocs/58`) | `:CollectionProperties` properties-node — folds template-info + default-FC-strategy + cross-cutting Collection config | user request, `aidocs/58 §5` | M | **design done** | `aidocs/58`. Idempotent migration `V##__Add_collection_properties.cypher`. CP1a–CP1c. |
...
| U1c2 | Role-in-current-context chip in page header (Owner / Editor / Reader + Instance Admin chip when applicable) | user request 2026-05-12, `aidocs/51 §9.4` | S | **design done** | `aidocs/51`. Surfaced via existing `PermissionsService.getRoles(entity)` + `profile=relations` response shape from `aidocs/56`; no new backend endpoint. Order: after CP1 (`aidocs/58 §5`) lands so the chip reads per-Collection defaults from `:CollectionProperties`. |
| A3b1 | Admin-page metrics strip — `/v2/admin/metrics-summary` + Vue cards linking to Grafana panels | user request 2026-05-12, `aidocs/51 §9.5` | M | **design done** | `aidocs/51`. Link-only v1 (server-side wraps a few `instant` Prometheus queries; no iframe / no anon Grafana access). Embed-via-iframe is A3b2, deferred. Gated `@RolesAllowed("instance-admin")` (A0 — shipped). |
