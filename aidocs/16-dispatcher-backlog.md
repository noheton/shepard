...
| L2 | Neo4J: stop using deprecated `id()` function, **migrate to application-generated IDs** (NOT `elementId()`) | 90, 715–717 | L | **needs design — direction set** | Design direction confirmed by maintainer 2026-05-05: do not migrate `id()` → `elementId()`, because Neo4j reuses internal IDs when nodes are deleted (the same brittleness applies to `elementId()`, plus `elementId()` only guarantees mapping within a single transaction and gives no length / ordering guarantees). Use **application-generated IDs** (UUID v4 / v7 / ULID — TBD per the design doc). Templates (**L3**) is the canonical use case that motivates a stable identifier independent of Neo4j's internal lifecycle: a template instance must be able to say "I'm an instance of template X" even after template re-import. Design doc to land at `aidocs/25-neo4j-id-migration-design.md`. |
