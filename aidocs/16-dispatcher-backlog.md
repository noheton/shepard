...
| C5 | Replace string-concatenated Cypher query construction in `Neo4jQueryBuilder.java`, `PermissionsDAO.java`, `DataObjectDAO.java` with parameterised queries | `aidocs/07-security-issues.md` C5, `aidocs/19-architecture-feedback.md` §2, `aidocs/25-neo4j-id-migration-design.md` §5 | M | done | Cherry-pick `e90bfd8`. New `Neo4jQuery(cypher, params)` record; `ParamBinder` threaded through every recursive `Neo4jQueryBuilder` helper. Property-name allowlist (`KNOWN_PROPERTIES` + `[A-Za-z_][A-Za-z0-9_|.]*` regex) sits next to `OP_PROPERTY`/`OP_VALUE`. **Subsumes M9** (createdBy/updatedBy `.toString().toLowerCase()` site fixed). 10-test injection regression suite (`Neo4jQueryBuilderInjectionTest`) covers payloads like `"name})RETURN n//"`, `"foo\"RETURN labels(n)//"`, IRI-with-quotes, plus numeric-id binding. **L2c precondition cleared.** Targeted suite green (47 + 10 + 5 = 62/62 on this branch); full `mvn -DskipITs verify` `Tests run: 1278, Failures: 0, Errors: 1, Skipped: 129` — single error is the pre-existing `oidc.public` baseline. |
| C5b | Apply C5's parameter-binding pattern to the second-wave `id()=` / `ID()=` sites — `GenericDAO`, `VersionDAO`, `ShepardFileDAO`, `StructuredDataDAO`, `*ReferenceDAO` family, `SemanticAnnotationDAO` | — | M | done | Cherry-pick `c707e56` (was `1827ebb` on worktree). 12 production files + 12 test files; new `Neo4jQuery` return type from `GenericDAO.getSearchForReachableReferences*`; all `findByDataObjectNeo4jId` / `findVersionLightByNeo4jId` / `findAllSemanticAnnotationsByNeo4jId` / `find` (file + structured) bind named parameters. Final exit-grep clean of user-reachable injection sites — only remaining matches are `Neo4jQueryBuilder.java:673` (already C5-bound via `binder.bind()`) and the historical `V2__Extract_json.java` migration (never re-run). Targeted suite green (62 + 6 skipped). **L2c is fully unblocked** — read-path swap can now flip parameter types from `long` to `String` without re-opening injection vectors. |
...
| L2b | Phase 2: Cypher backfill `appId = randomUUID()`, idempotent | — | S | done | Cherry-pick `796bc11` (was `541884f` on worktree). `V12__Backfill_appId.cypher` — 28 per-label `MATCH … WHERE n.appId IS NULL CALL { WITH n SET n.appId = randomUUID() } IN TRANSACTIONS OF 10000 ROWS;` statements. Verified by class-file inspection that `neo4j-migrations 3.2.1` routes `IN TRANSACTIONS` clauses through implicit transactions even under `MigrationsConfig.TransactionMode.PER_STATEMENT` — no fall-back to bare per-label statements needed. Operator-run rollback at `V12_R__Rollback_Backfill_appId.cypher` (file naming intentionally outside the runner's `VERSION_PATTERN`; documented `cypher-shell -f` invocation in the file header). Test added at `TestNeo4jMigrations.testV12` covering null-seed → V11 → V12 → null-count = 0, label coverage, uniqueness, and idempotency (re-run is a no-op). Per-chunk `MigrationProgress` Neo4j-side schema deferred — would conflict with P3's TimescaleDB-side pattern. |
| L2c | Phase 3: switch every Cypher to `WHERE e.appId = $appId` | — | L | done | Cherry-pick TBD. New `EntityIdResolver` `@RequestScoped` bean (`backend/src/main/java/de/dlr/shepard/common/identifier/EntityIdResolver.java`) translates the OGM Long ↔ `appId` at the DAO boundary; injected on `GenericDAO`'s base. DAO Cypher flipped from `WHERE ID(e) = $entityId` (`long`) to `WHERE e.appId = $appId` (`String`) across `PermissionsDAO`, `DataObjectDAO`, `GenericDAO.getSearchForReachableReferences*`, `BasicReferenceDAO`, `FileReferenceDAO`, `URIReferenceDAO`, `StructuredDataReferenceDAO`, `TimeseriesReferenceDAO`, `CollectionReferenceDAO`, `DataObjectReferenceDAO`, `VersionDAO.findVersionLightByNeo4jId`/`createLink`/`copyInternalDataObjectReferences`, `ShepardFileDAO.find`, `StructuredDataDAO.find`, `SemanticAnnotationDAO.findAllSemanticAnnotationsByNeo4jId`. Public DAO method signatures stay `long` for caller-compat — the resolver runs internally. Permission cache key stays `long` (per `aidocs/25 §3.3`); `PermissionsService.isAllowed` segment dispatch **deliberately not touched** (that's L2d, when `/v2/` paths arrive with UUID segments — touching it now without `/v2/` routes would never exercise the new branch). `Neo4jQueryBuilder`'s search-JSON-derived numeric-id predicates stay (changing them is a public-API-shape change, also L2d). 9-test `EntityIdResolverTest` covers round-trip + memoisation + not-found paths. Existing DAO unit tests updated to assert the new Cypher shape (16 test files touched). Full suite `Tests run: 1288, Failures: 0, Errors: 1, Skipped: 128` — single error `ReferenceSearchServiceQuarkusTest.findExactlyOneReferenceByeIRIEquality` is the pre-existing Postgres-availability baseline. Coverage 69.05% line / 66.25% branch (above the 60/60 floor). SpotBugs: 0 findings. |
