...
| H4 | Surface RFC 7807-shape error responses (existing high-finding from `07`) | `aidocs/07-security-issues.md` H4, `aidocs/19-architecture-feedback.md` §3 | M | done | Cherry-pick `e526183`. New `application/problem+json` shape via `ProblemJson` record (RFC 7807 §3.1 + flat `extensions` map via `@JsonAnyGetter`); 10-entry `ShepardErrorCode` catalogue; mapper rewritten — known shepard exceptions surface their message as `detail`, unknown-5xx returns generic `"An internal server error occurred. Reference: <traceId>"`. Content negotiation: legacy `ApiError` returned only on explicit `Accept: application/json` (no problem+json, no wildcard); everything else gets the new shape. `instance` field from `X-Request-Id` header or request-scoped `TraceIdProvider` UUID. **Subsumes M7**: full stack at `debug`; error line carries only `traceId + class + method + path`. 20 mapper tests + 2 trace-id tests. **L2d fully unblocked** (P4 routing scaffolding shipped at `e0c5e32`; H4 was the last gate). |
