...
| P21 | Introduce PATCH for partial-update endpoints (currently every update is PUT despite partial DTOs — `DataObjectRest:162`, `TimeseriesRest:472`, etc.) | M | **pilot done** | Surfaced by `aidocs/26-crud-consistency.md` §3 finding #1: **0 PATCH endpoints across 153**. Strategy choice: ship PATCH additively in `/v1/` and deprecate the partial-PUT semantics in `/v2/` (cleanest of three). **Pilot landed for `Collection`** at commit `10531bd` — `@PATCH /collections/{collectionId}` with RFC 7396 JSON Merge Patch via Jackson `readerForUpdating`; PUT unchanged for `/v1/` backwards compat. Accepts both `application/merge-patch+json` and `application/json`; future `/v2/` will require the merge-patch content type. Bean Validation runs on the *merged* result. 6 IT cases in `CollectionIT.java` (happy / null / empty / invalid-merge / no-WRITE / json-content-type). Per-resource follow-ups: P21-DataObject, P21-File, P21-StructuredData, etc. each ~½–1 day once a maintainer signs off the pilot's shape. |
