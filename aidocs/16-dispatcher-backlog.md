...
| U2a | `GitCredential` entity + CRUD `/v2/me/git-credentials` + AES-GCM PAT encryption — **done** | — | S | **done** | `GitCredential` Neo4j entity (`OWNS_CREDENTIAL` rel from `:User`); `AesGcmCipher` utility (AES-256-GCM, random 12-byte IV, base64 encoded); `SecretsKeyValidator` startup check (`shepard.secrets.encryption-key` optional 32-byte base64 key); `GitCredentialDAO` (owner-scoped Cypher); `MeCredentialsRest` at `/v2/me/git-credentials` (GET list, POST create 201+Location, GET single, PATCH, DELETE); 501 when key absent; PAT never returned on wire. `V20__Add_appId_constraint_GitCredential.cypher` (idempotent). 16 unit tests (`MeCredentialsRestTest`) + 6 cipher tests (`AesGcmCipherTest`). Unblocks G1b (`git.pat` storage). |
...
| J1a | Markdown body interpretation (CommonMark + GFM, sanitised) + `GET /v2/lab-journal/{appId}/render`. | — | S | **done** | `MarkdownRenderer` (commonmark 0.22.0 + jsoup `Safelist.relaxed()`); `LabJournalRenderRest` at `/v2/lab-journal/{appId}/render`; `EntityIdResolver.resolveLong` + `PermissionsService.isAccessTypeAllowedForUser` (Read on owning Collection); 6-test unit suite. No data migration; CommonMark passes plain text unchanged. |
