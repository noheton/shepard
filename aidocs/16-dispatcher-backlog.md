...
| C5 | Replace string-concatenated Cypher query construction in `Neo4jQueryBuilder.java`, `PermissionsDAO.java`, `DataObjectDAO.java` with parameterised queries | `aidocs/07-security-issues.md` C5, `aidocs/19-architecture-feedback.md` §2, `aidocs/25-neo4j-id-migration-design.md` §5 | M | done | Cherry-pick `ab3f9da`. New `Neo4jQuery(cypher, params)` record; `ParamBinder` threaded through every recursive `Neo4jQueryBuilder` helper. Property-name allowlist (`KNOWN_PROPERTIES` + `[A-Za-z_][A-Za-z0-9_|.]*` regex) sits next to `OP_PROPERTY`/`OP_VALUE`. **Subsumes M9** (createdBy/updatedBy `.toString().toLowerCase()` site fixed). 10-test injection regression suite (`Neo4jQueryBuilderInjectionTest`) covers payloads like `"name})RETURN n//"`, `"foo\"RETURN labels(n)//"`, IRI-with-quotes, plus numeric-id binding. **L2c precondition cleared.** Targeted suite green (47 + 10 + 5 = 62/62 on this branch); full `mvn -DskipITs verify` `Tests run: 1278, Failures: 0, Errors: 1, Skipped: 129` — single error is the pre-existing `oidc.public` baseline. |
| C5b | Apply the same parameter-binding pattern to the additional `id()=` / `ID()=` concatenation sites surfaced by C5's final grep — `GenericDAO.getSearchForReachableReferences*:170/190/208`, `VersionDAO:74`, `ShepardFileDAO:22`, `StructuredDataDAO:22`, `*ReferenceDAO` family (`file/uri/structureddata/basicreference/timeseriesreference/dataobject`), `SemanticAnnotationDAO:18` | — | M | queued | Same shape as C5 — could be a single targeted refactor across the `*ReferenceDAO` family. Not bundled with C5 to keep the security fix tightly scoped. **Should land before L2c** so the L2c parameter-type swap (long → String) doesn't open new injection sites. |
