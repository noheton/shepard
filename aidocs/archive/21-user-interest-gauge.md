...
Companion to `aidocs/semantics/14-semantic-improvements.md` (already proposes
the KG path), `aidocs/semantics/13-search-improvements.md` (where these would
be queried), `aidocs/16-dispatcher-backlog.md` (live backlog) and
`aidocs/archive/03-issues-status.md` (the richest signal source).
...
| **Knowledge-graph interfaces** | Strongest evidence of the three. Active Slack/Mattermost discussions on Fuseki / SPARQL (`input_raw.md:2918-2976, 4874`), four open GitLab issues (#43 / #553 / #656 / #660 — `aidocs/archive/03-issues-status.md:70-85`), an existing partial implementation (`SparqlConnector.java`), a written design proposal (`aidocs/semantics/14-semantic-improvements.md`), and explicit demand from at least three institutes (BT-Augsburg MEMAS, turbine-data department, Damast project). |
...
- 166 open GitLab issues with effort/value/staleness scores
  (`aidocs/archive/03-issues-status.md`).
...
- Maintainer-curated design notes (`aidocs/semantics/14-semantic-improvements.md`,
...
- **No open GitLab issue** mentions `hdf5` or `hsds`
  (`aidocs/archive/03-issues-status.md` covers all 166 open items; none
  match this cluster).
- **TimescaleDB satisfies the obvious overlap.** Numeric, multi-
  ...
  `aidocs/data/12-timescaledb-performance-analysis.md`). The remaining
...
- **Operational cost.** HSDS is a full extra service (object
  ...
  Caddy + Keycloak + 4 DBs (`aidocs/archive/01-repo-overview.md:48`).
...
- New auth bridge — HSDS has its own auth model; would need to
  ...
  (`aidocs/archive/01-repo-overview.md:49`).
...
(`aidocs/platform/20-epic-roadmap.md:309-329`).
...
- **Search-improvements doc §4** already proposes a
  ...
  (`aidocs/semantics/13-search-improvements.md`) — also pulls toward
...
- **No open GitLab issue** mentions `table store`, `tabular`,
  `relational storage`, or similar (`aidocs/archive/03-issues-status.md`).
...
- **TimescaleDB already gives users SQL over timeseries** for
  ...
  (`aidocs/archive/01-repo-overview.md:48`). That is the lowest-cost
...
`aidocs/semantics/14-semantic-improvements.md` already enumerates the
...
- **(i) SPARQL endpoint over the existing Neo4j data.** No new
  ...
  (neosemantics) or a thin façade. `aidocs/semantics/14-semantic-improvements.md:362-368`
...
- **(ii) Full triplestore** (RDF4J / GraphDB / Fuseki) as a new
  ...
  zone (`aidocs/semantics/14-semantic-improvements.md:341-359`).
...
- **Open GitLab issues #43, #553, #656, #660** — semantic-
  ...
  (`aidocs/archive/03-issues-status.md:70-85`). #43 is PARTIAL —
...
- **Maintainer design proposal** —
  `aidocs/semantics/14-semantic-improvements.md` (572 lines) is itself the
  strongest signal: a maintainer has already invested in writing
  this up rather than parking it. Phase D in the roadmap
  (`aidocs/semantics/14-semantic-improvements.md:507-518`) is the n10s
...
- The four open semantic-annotation issues are **mid-staleness**
  ...
  `aidocs/archive/03-issues-status.md:70-85`). Demand exists but has
  not turned into a closed feature in over a year.
- **#274 (Neo4j refactor) is the load-bearing blocker** for the
  whole cluster (`aidocs/archive/03-issues-status.md:79-85`,
  `aidocs/semantics/14-semantic-improvements.md:90-100`). KG work needs
...
- **RO-Crate import** (export already exists —
  `aidocs/archive/01-repo-overview.md:77`). `input_raw.md:7982, 8338`.
  HMC2 candidate.
...
| 8 | xit-* contractor handover lead (`aidocs/archive/03-issues-status.md:128`) | "What was the original demand behind the long-tail research issues you owned, and is it still live?" |
...
- `aidocs/16-dispatcher-backlog.md` — promote any area that
  ...
  clear no threshold.
- `aidocs/platform/20-epic-roadmap.md` — slot the survivors into the
  appropriate epic. KG: align with Phases A–H of
  `aidocs/semantics/14-semantic-improvements.md:507-518`. HDF5 /
  Tabular: write a fresh epic, weighed against C-level
  security findings (`aidocs/archive/07-security-issues.md`) which
...
- **Production usage analytics** — request volumes per endpoint,
  ...
  (`aidocs/archive/01-repo-overview.md:78`) but the metric history
  is not in the repo.
...
- **Adjacent project user lists** (Damast, MOSS, dataship,
  ...
  count.
- **GitHub mirror traffic.** `aidocs/archive/04-reconciliation.md`
...
- Adjacent asks (AAS, NovaCrate, ORCID, RO-Crate, MCP):
  ...
  8046, 8065-8066, 8389`.
- Open issues cluster: `aidocs/archive/03-issues-status.md:70-85`.
...
- Companion design notes: `aidocs/semantics/14-semantic-improvements.md`,
  `aidocs/semantics/13-search-improvements.md`,
  `aidocs/data/12-timescaledb-performance-analysis.md`.
- Backlog targets: `aidocs/16-dispatcher-backlog.md`,
  `aidocs/platform/20-epic-roadmap.md` (registration handled by
